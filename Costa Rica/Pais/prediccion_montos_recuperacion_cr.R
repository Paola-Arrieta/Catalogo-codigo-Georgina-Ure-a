...
# # Se tienen observaciones desde 2000-03-19 a hoy (no equiespaciadas o consecutivas)
...
# por columna
# apply(cr_pagos, 2, porcentaje_empty)
# 6.29% de la data en la que no hubo recuperación.
# # Se podría trabajar desde 2001-10-01, ya que a partir de ahí la data está completa hasta la fecha actual.
...
# head(serie)
# tail(serie)
# El objeto creado anteriormente inicia en 634000 como monto de pago por todos los clientes de Costa Rica y finaliza en 1.107.669.289 en setiembre 2023. Es decir, es una sucesión ordenada de valores.
...
# # Se obtiene un valor de -1.8735 y un p-value de 0.6292. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.  
...
# frequencies
# # 0.17777778 0.13333333 0.08888889
# 
...
# model_arima = arima(serie_training, order = c(0,1,3), seasonal = list(order = c(0,0,0), period = 12))
# model_arima
# 
# 
