# ---
# title: "Predicción de montos recuperación Servicoop"
...
cr_pagos_servicoop_data = dbGetQuery(connection, "
...
# str(cr_pagos_servicoop_data)
cr_pagos_servicoop_data$FECHA_PAGO = as.Date(cr_pagos_servicoop_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(cr_pagos_servicoop_data)
# # Se tienen observaciones desde 2017-03-01 hasta 2023-10-01 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

cr_pagos_servicoop = cr_pagos_servicoop_data %>% 
...
# # por columna
# apply(cr_pagos_servicoop, 2, porcentaje_empty)
# # 5% de la data en la que no hubo recuperación (data casi completa).

# Se completa la primera columna
cr_pagos_servicoop$INSTITUCION = replace_na(as.character(cr_pagos_servicoop$INSTITUCION), "SERVICOOP")
# Se interpolan los valores perdidos
cr_pagos_servicoop = cr_pagos_servicoop %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# El primer registro es 2017-03. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = cr_pagos_servicoop$FECHA_PAGO[1]
...
# residuos_data = diff(cr_pagos_servicoop$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.0000000000006917) < (alfa = 0.05)].
# 
...
serie = ts(cr_pagos_servicoop$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -2.6263 y un p-value de = 0.3196. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 2.857143 3.333333 3.076923
# # Es así como de determina que los períodos más importantes son cada 2.8 meses, cada 3.33 meses y cada 3.07 meses.
# 
...
# model_arima = arima(serie_training, order = c(3,0,0), seasonal = list(order = c(0,0,0), period = 12))
# model_arima
# 
...
# model_calibrated_arima = arima(serie_training, order = c(0, 4, 4), seasonal = list(order = c(1, 0, 0), period = 12))
...
final_date = cr_pagos_servicoop$FECHA_PAGO[nrow(cr_pagos_servicoop)]
final_date
...
total_dates = c(cr_pagos_servicoop$FECHA_PAGO, dates)
...
predicciones_finales_servicoop = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_servicoop) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_servicoop = predicciones_finales_servicoop %>%
...
saveRDS(predicciones_finales_servicoop, "predicciones_finales_servicoop.rds")
