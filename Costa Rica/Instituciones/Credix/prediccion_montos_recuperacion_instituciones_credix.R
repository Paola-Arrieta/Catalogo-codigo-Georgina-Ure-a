# ---
# title: "Predicción de montos recuperación Credix"
...
cr_pagos_credix_data = dbGetQuery(connection, "
...
# str(cr_pagos_credix_data)
cr_pagos_credix_data$FECHA_PAGO = as.Date(cr_pagos_credix_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(cr_pagos_credix_data)
# # Se tienen observaciones desde 2017-09 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

cr_pagos_credix = cr_pagos_credix_data %>% 
...
# por columna
# apply(cr_pagos_credix, 2, porcentaje_empty)
# 0% de la data en la que no hubo recuperación (data completa).

# Se completa la primera columna
cr_pagos_credix$INSTITUCION = replace_na(as.character(cr_pagos_credix$INSTITUCION), "CREDIX")
# Se interpolan los valores perdidos
cr_pagos_credix = cr_pagos_credix %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# El primer registro es 2017-09-01, pero se empieza la serie al mes siguiente. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = '2017-10-01'
...
# residuos_data = diff(cr_pagos_credix$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.9114) > (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.9217) > (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.8458) > (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(cr_pagos_credix$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -1.6987 y un p-value de = 0.6981. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 37.50 15.00 18.75
# # Es así como de determina que los períodos más importantes son cada 37.5 meses (poco más de cada 3 años), cada 15 meses (más de un año) y  cada 18.75 meses (cada año y medio).
# 
...
# model_arima = arima(serie_training, order = c(1,1,2), seasonal = list(order = c(0,0,0), period = 12))
# model_arima
# 
...
# model_calibrated_arima = arima(serie_training, order = c(4, 3, 5), seasonal = list(order = c(0, 1, 0), period = 12))
...
final_date = cr_pagos_credix$FECHA_PAGO[nrow(cr_pagos_credix)]
final_date
...
total_dates = c(cr_pagos_credix$FECHA_PAGO, dates)
...
predicciones_finales_credix = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_credix) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_credix = predicciones_finales_credix %>%
...
saveRDS(predicciones_finales_credix, "predicciones_finales_credix.rds")
