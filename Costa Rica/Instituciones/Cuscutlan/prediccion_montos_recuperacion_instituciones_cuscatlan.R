# ---
# title: "Predicción de montos recuperación Cuscatlan"
...
cr_pagos_cuscatlan_data = dbGetQuery(connection, "
...
# str(cr_pagos_cuscatlan_data)
cr_pagos_cuscatlan_data$FECHA_PAGO = as.Date(cr_pagos_cuscatlan_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(cr_pagos_cuscatlan_data)
# # Se tienen observaciones desde 2003-11 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

cr_pagos_cuscatlan = cr_pagos_cuscatlan_data %>% 
...
# # por columna
# apply(cr_pagos_cuscatlan, 2, porcentaje_empty)
# # 2.08% de la data en la que no hubo recuperación (casi data completa).

# Se completa la primera columna
cr_pagos_cuscatlan$INSTITUCION = replace_na(as.character(cr_pagos_cuscatlan$INSTITUCION), "CUSCATLAN")
# Se interpolan los valores perdidos
cr_pagos_cuscatlan = cr_pagos_cuscatlan %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# El primer registro es 2003-11. Decido dejar como primera observación de la serie 2015-01-01 para mayor consistencia y como última observación el mes anterior al actual completo.  
init_first_month = '2015-01-01'
...
# residuos_data = diff(cr_pagos_cuscatlan$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.000000000737) < (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(cr_pagos_cuscatlan$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -2.8964 y un p-value de = 0.1989. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 120.000000  80.000000   3.870968
# # Es así como de determina que los períodos más importantes son cada 120.0 meses (cada 10 años), cada 80 meses (poco más de 6 años y medio) y  cada 3.87 meses (cada casi cuatrimestre).
# 
...
# model_calibrated_arima = arima(serie_training, order = c(3, 2, 0), seasonal = list(order = c(0, 0, 0), period = 12))
...
# 
# errors
# # El cuadro detalla los índices de error para cada uno de los modelos planteados. De esta manera se confirma que Arima calibrado no es la mejor opción, ya que sus valores en MSE y RMSE son muy altos, mientras que Holt-Winters calibrado muestra lo contrario (inclusive ARIMA y Holt-Winters sin calibrar).
# 
...
final_date = cr_pagos_cuscatlan$FECHA_PAGO[nrow(cr_pagos_cuscatlan)]
final_date
...
total_dates = c(cr_pagos_cuscatlan$FECHA_PAGO, dates)
...
#   dyAxis("y", valueRange = c(0, 5500000))
...
predicciones_finales_cuscatlan = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_cuscatlan) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_cuscatlan = predicciones_finales_cuscatlan %>%
...
saveRDS(predicciones_finales_cuscatlan, "predicciones_finales_cuscatlan.rds")
