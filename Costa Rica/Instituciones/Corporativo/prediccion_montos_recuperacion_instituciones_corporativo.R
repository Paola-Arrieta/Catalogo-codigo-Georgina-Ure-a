# ---
# title: "Predicción de montos recuperación Corporativo"
...
cr_pagos_corporativo_data = dbGetQuery(connection, "
...
# str(cr_pagos_corporativo_data)
cr_pagos_corporativo_data$FECHA_PAGO = as.Date(cr_pagos_corporativo_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(cr_pagos_corporativo_data)
# Se tienen observaciones desde 2011-05 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

cr_pagos_corporativo = cr_pagos_corporativo_data %>% 
...
# # por columna
# apply(cr_pagos_corporativo, 2, porcentaje_empty)
# # 44.6% de la data en la que no hubo recuperación. Ante esto, decido acortar la serie y empezarla en marzo del 2019, ya que se cuenta con más líneas completas.

cr_pagos_corporativo = cr_pagos_corporativo %>%
  filter((FECHA_PAGO >= '2019-03-01'))
# porcentaje_empty = function(x) {sum(is.na(x)) / length(x)*100}
# por columna
# apply(cr_pagos_corporativo, 2, porcentaje_empty)
# Los valores de no recuperación pasan a ser 8.9%.

# Se completa la primera columna
cr_pagos_corporativo$INSTITUCION = replace_na(as.character(cr_pagos_corporativo$INSTITUCION), "CORPORATIVO")
# Se interpolan los valores perdidos
cr_pagos_corporativo = cr_pagos_corporativo %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# El primer registro es 2019-03-01. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = '2019-03-01'
...
# residuos_data = diff(cr_pagos_corporativo$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.000003233) < (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.000005236) < (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.00000001588) < (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(cr_pagos_corporativo$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -2.561 y un p-value de = 0.3485. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 30 20 15
# # Es así como de determina que los períodos más importantes son cada 30 meses, cada 20 meses y cada 15 meses.
# 
...
# model_calibrated_arima = arima(serie_training, order = c(5, 1, 0), seasonal = list(order = c(0, 0, 0), period = 12))
...
final_date = cr_pagos_corporativo$FECHA_PAGO[nrow(cr_pagos_corporativo)]
final_date
...
total_dates = c(cr_pagos_corporativo$FECHA_PAGO, dates)
...
predicciones_finales_corporativo = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_corporativo) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_corporativo = predicciones_finales_corporativo %>%
...
saveRDS(predicciones_finales_corporativo, "predicciones_finales_corporativo.rds")
