# ---
# title: "Predicción de montos recuperación Davivienda"
...
cr_pagos_davivienda_data = dbGetQuery(connection, "
...
# str(cr_pagos_davivienda_data)
cr_pagos_davivienda_data$FECHA_PAGO = as.Date(cr_pagos_davivienda_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(cr_pagos_davivienda_data)
...
cr_pagos_davivienda = cr_pagos_davivienda_data %>% 
...
# # por columna
# apply(cr_pagos_davivienda, 2, porcentaje_empty)
# # 0% de la data en la que no hubo recuperación (data completa).

# Se completa la primera columna
cr_pagos_davivienda$INSTITUCION = replace_na(as.character(cr_pagos_davivienda$INSTITUCION), "DAVIVIENDA")
# Se interpolan los valores perdidos
cr_pagos_davivienda = cr_pagos_davivienda %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# El primer registro es 2019-12-01; sin embargo, para mayor consistencia de la data, decido empezar la serie el mes siguiente. Decido dejar como última observación el mes anterior al actual completo.  
...
# residuos_data = diff(cr_pagos_davivienda$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.4524) > (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.1222) > (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.1709) > (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(cr_pagos_davivienda$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -1.4614 y un p-value de = 0.7881. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 24 12 16
# # Es así como de determina que los períodos más importantes son cada 24 meses (cada 2 años), cada 12 meses (cada año) y cada 16 meses (cada poco más de un año).
# 
...
# model_arima = arima(serie_training, order = c(0,1,1), seasonal = list(order = c(0,1,0), period = 12))
# model_arima
# 
...
# model_calibrated_arima = arima(serie_training, order = c(2, 2, 0), seasonal = list(order = c(0, 1, 0), period = 12))
...
final_date = cr_pagos_davivienda$FECHA_PAGO[nrow(cr_pagos_davivienda)]
final_date
...
total_dates = c(cr_pagos_davivienda$FECHA_PAGO, dates)
...
#   dyAxis("y", valueRange = c(0, 160000000))
