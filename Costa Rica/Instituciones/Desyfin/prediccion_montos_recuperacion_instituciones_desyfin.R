# ---
# title: "Predicción de montos recuperación Desyfin"
...
cr_pagos_desyfin_data = dbGetQuery(connection, "
...
# str(cr_pagos_desyfin_data)
cr_pagos_desyfin_data$FECHA_PAGO = as.Date(cr_pagos_desyfin_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(cr_pagos_desyfin_data)
# Se tienen observaciones desde 2016-09-01  hasta 2021-06-01 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

cr_pagos_desyfin = cr_pagos_desyfin_data %>% 
...
# por columna
# apply(cr_pagos_desyfin, 2, porcentaje_empty)
# 1.7% de la data en la que no hubo recuperación (casi data completa).

# Se completa la primera columna
cr_pagos_desyfin$INSTITUCION = replace_na(as.character(cr_pagos_desyfin$INSTITUCION), "DESYFIN")
# Se interpolan los valores perdidos
cr_pagos_desyfin = cr_pagos_desyfin %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# El primer registro es 2017-09-01. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = cr_pagos_desyfin$FECHA_PAGO[1]
...
# residuos_data = diff(cr_pagos_desyfin$MONTO_TOTAL)
# 
...
# # El histograma anterior presenta la distribución de los residuos para las observaciones. Se observa una distribución medio asimétrica, lo que podría indicar que tal vez incluye valores atípicos.
# 
...
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value < 0.00000000000000022) < (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values < 0.00000000000000022) < (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.000000000737) < (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(cr_pagos_desyfin$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -3.6609 y un p-value de = 0.03587. La hipótesis nula es no aceptada y, por lo tanto, la serie es estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 2.727273 20.000000 30.000000
# # Es así como de determina que los períodos más importantes son cada 2.7 meses, cada 20 meses (cada casi dos años) y cada 30 meses (cada dos años y medio).
# 
...
# model_arima = arima(serie_training, order = c(0,0,0), seasonal = list(order = c(0,0,0), period = 12))
# model_arima
# 
...
# model_calibrated_arima = arima(serie_training, order = c(0, 2, 1), seasonal = list(order = c(1, 0, 0), period = 12))
# model_calibrated_arima
# 
# # Predicciones con Arima calibrado
# pred_calibrated_arima = predict(model_calibrated_arima, n.ahead = 12)
...
# 
# errors
# # El cuadro detalla los índices de error para cada uno de los modelos planteados. De esta manera se confirma que Arima no es la mejor opción, ya que sus valores en MSE y RMSE son muy altos, mientras que Holt-Winters calibrado muestra lo contrario.
# 
...
# De esta manera se muestran las predicciones de los que hubieran sido los últimos meses del 2021, así como sus respectivos intervalos con 95% de confianza.
...
final_date = cr_pagos_desyfin$FECHA_PAGO[nrow(cr_pagos_desyfin)]
final_date
...
total_dates = c(cr_pagos_desyfin$FECHA_PAGO, dates)
...
#   dyAxis("y", valueRange = c(0, 35000000))
...
predicciones_finales_desyfin = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_desyfin) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_desyfin = predicciones_finales_desyfin %>%
...
saveRDS(predicciones_finales_desyfin, "predicciones_finales_desyfin.rds")
