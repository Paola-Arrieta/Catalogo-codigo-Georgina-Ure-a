# ---
# title: "Predicción de montos recuperación Capicredit"
...
cr_pagos_capicredit_data = dbGetQuery(connection, "
...
# str(cr_pagos_capicredit_data)
cr_pagos_capicredit_data$FECHA_PAGO = as.Date(cr_pagos_capicredit_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(cr_pagos_capicredit)
# # Se tienen observaciones desde 2018-04 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

cr_pagos_capicredit = cr_pagos_capicredit_data %>% 
...
# # por columna
# apply(cr_pagos_capicredit, 2, porcentaje_empty)
# # 22.05% de la data en la que no hubo recuperación.

# Se completa la primera columna
cr_pagos_capicredit$INSTITUCION = replace_na(as.character(cr_pagos_capicredit$INSTITUCION), "CAPICREDIT")
# Se interpolan los valores perdidos
cr_pagos_capicredit = cr_pagos_capicredit %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# El primer registro es 2018-04; no obstante, decido utilizar como inicio de serie 2019-03-01 para que los datos interpolados sean mínimos. Decido dejar como última observación el mes anterior al actual completo.  
...
# residuos_data = diff(cr_pagos_capicredit$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.04652) < (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.01653) < (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.0003233) < (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(cr_pagos_capicredit$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -1.6127 y un p-value de 0.7322. La hipótesis nula es no aceptada y, por lo tanto, la serie es estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 24.000000 36.000000  5.142857
# # Es así como de determina que los períodos más importantes son cada 24.0 meses, cada 36.0 meses y cada 5.1 meses.
# 
...
# model_calibrated_arima = arima(serie_training, order = c(0, 0, 0), seasonal = list(order = c(0, 1, 1), period = 12))
...
final_date = cr_pagos_capicredit$FECHA_PAGO[nrow(cr_pagos_capicredit)]
final_date
...
total_dates = c(cr_pagos_capicredit$FECHA_PAGO, dates)
...
predicciones_finales_capicredit = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_capicredit) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_capicredit = predicciones_finales_capicredit %>%
...
saveRDS(predicciones_finales_capicredit, "predicciones_finales_capicredit.rds")
