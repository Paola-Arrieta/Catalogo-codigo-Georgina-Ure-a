...
cr_pagos_promerica_data = dbGetQuery(connection, "
...
# str(cr_pagos_promerica_data)
cr_pagos_promerica_data$FECHA_PAGO = as.Date(cr_pagos_promerica_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(cr_pagos_promerica_data)

cr_pagos_promerica = cr_pagos_promerica_data %>% 
...
# # por columna
# apply(cr_pagos_promerica, 2, porcentaje_empty)
# # 0% de la data en la que no hubo recuperación (data completa).

# Se completa la primera columna
cr_pagos_promerica$INSTITUCION = replace_na(as.character(cr_pagos_promerica$INSTITUCION), "PROMERICA")
# Se interpolan los valores perdidos
cr_pagos_promerica = cr_pagos_promerica %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# El primer registro es 2001-10 con un monto total de 621.000. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = cr_pagos_promerica$FECHA_PAGO[1]
...
# residuos_data = diff(cr_pagos_promerica$MONTO_TOTAL)
# 
...
serie = ts(cr_pagos_promerica$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -1.33 y un p-value de 0.8581. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.  
...
# frequencies
# # 0.13333333 0.17777778 0.08888889
# 
...
# periods
# # 90.0  67.5 135.0
# # Es así como de determina que los períodos más importantes son cada 90.0 meses, cada 67.5 meses y cada 135.0 meses.
# 
...
# model_arima = arima(serie_training, order = c(0,1,2), seasonal = list(order = c(0,0,0), period = 12))
# model_arima
# 
...
# De esta manera se muestran las predicciones para los últimos meses de este 2023 y el primer cutrimestre del 2024, así como sus respectivos intervalos con 95% de confianza.
...
final_date = cr_pagos_promerica$FECHA_PAGO[nrow(cr_pagos_promerica)]
final_date
...
total_dates = c(cr_pagos_promerica$FECHA_PAGO, dates)
...
#   dyAxis("y", valueRange = c(0, 1200000000))
