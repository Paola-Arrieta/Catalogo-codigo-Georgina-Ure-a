# ---
# title: "Predicción de montos recuperación Danissa"
...
cr_pagos_danissa_data = dbGetQuery(connection, "
...
# str(cr_pagos_danissa_data)
cr_pagos_danissa_data$FECHA_PAGO = as.Date(cr_pagos_danissa_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(cr_pagos_danissa_data)
# # Se tienen observaciones desde 2018-03 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

cr_pagos_danissa = cr_pagos_danissa_data %>% 
...
# # por columna
# apply(cr_pagos_danissa, 2, porcentaje_empty)
# # 4.61% de la data en la que no hubo recuperación (casi data completa).

# Se completa la primera columna
cr_pagos_danissa$INSTITUCION = replace_na(as.character(cr_pagos_danissa$INSTITUCION), "DANISSA")
# Se interpolan los valores perdidos
cr_pagos_danissa = cr_pagos_danissa %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))


# El primer registro es 2018-03-01, pero, para mayor consistencia de los datos, se decide empezar en 2018-04-01. Como última observación queda el último mes del cual se tiene registro.  
init_first_month = '2018-04-01'
...
# residuos_data = diff(cr_pagos_danissa$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.2479) > (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.0437) < (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = = 0.003004) < (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(cr_pagos_danissa$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
# 
...
# # Se obtiene un valor de -2.3354 y un p-value de = 0.439. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 24 18 9
# # Es así como de determina que los períodos más importantes son cada 24 meses (cada 2 años), cada 18 meses (poco año y medio) y cada 9 meses.
# 
...
# model_calibrated_arima = arima(serie_training, order = c(0, 3, 2), seasonal = list(order = c(0, 0, 0), period = 12))
...
# 
# errors
# # El cuadro detalla los índices de error para cada uno de los modelos planteados. De esta manera se confirma que Holt-Winters no es la mejor opción, ya que sus valores en MSE y RMSE son muy altos, mientras que Holt-Winters calibrado muestra lo contrario (inclusive ARIMA).
# 
...
# 
# # Se confirma, nuevamente, que el mejor modelo es Holt-Winters calibrado (seguido del ARIMA) porque muestra valores más bajos para MSE y RMSE y PTFA.
...
# Predicciones con Holt-Winters calibrado
final_pred = forecast(final_model, h = 8, level = c(95))
final_pred
...
final_date = cr_pagos_danissa$FECHA_PAGO[nrow(cr_pagos_danissa)]
final_date

# Nuevas fechas para las predicciones
dates = final_date + months(1:8)
dates

# Unión de las nuevas fechas con las fechas de la serie original
total_dates = c(cr_pagos_danissa$FECHA_PAGO, dates)
...
#   dyAxis("y", valueRange = c(0, 170000000))
...
predicciones_finales_danissa = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_danissa) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_danissa = predicciones_finales_danissa %>%
...
saveRDS(predicciones_finales_danissa, "predicciones_finales_danissa.rds")
