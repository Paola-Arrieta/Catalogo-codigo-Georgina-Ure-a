# ---
# title: "Predicción de montos recuperación Legal"
...
cr_pagos_legal_data = dbGetQuery(connection, "
...
# str(cr_pagos_legal_data)
cr_pagos_legal_data$FECHA_PAGO = as.Date(cr_pagos_legal_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(cr_pagos_legal_data)
...
cr_pagos_legal = cr_pagos_legal_data %>% 
...
# # por columna
# apply(cr_pagos_legal, 2, porcentaje_empty)
# # 0% de la data en la que no hubo recuperación.

# Se completa la primera columna
cr_pagos_legal$INSTITUCION = replace_na(as.character(cr_pagos_legal$INSTITUCION), "Legal")
# Se interpolan los valores perdidos
cr_pagos_legal = cr_pagos_legal %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# El primer registro es 2012-09-01. No obstante, la serie va a empezar el 2012-10-01 para mayor consistencia de la data. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = '2012-10-01'
...
# residuos_data = diff(cr_pagos_legal$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.00000000000133) < (alfa = 0.05)].
# 
...
serie = ts(cr_pagos_legal$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -4.2581 y un p-value de = 0.01. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 3.068182 45.000000  2.288136
# # Es así como de determina que los períodos más importantes son cada 3.06 meses, cada 45 meses (poco menos de cada 4 años) y cada 2.28 meses.
# 
...
# model_arima = arima(serie_training, order = c(0,0,0), seasonal = list(order = c(0,0,1), period = 12))
# model_arima
# 
...
# model_calibrated_arima = arima(serie_training, order = c(2, 3, 1), seasonal = list(order = c(0, 0, 0), period = 12))
...
final_date = cr_pagos_legal$FECHA_PAGO[nrow(cr_pagos_legal)]
final_date
...
total_dates = c(cr_pagos_legal$FECHA_PAGO, dates)
...
predicciones_finales_legal = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_legal) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_legal = predicciones_finales_legal %>%
...
saveRDS(predicciones_finales_legal, "predicciones_finales_legal.rds")
