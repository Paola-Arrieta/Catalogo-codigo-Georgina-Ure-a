# ---
# title: "Predicción de montos recuperación Cathay"
...
cr_pagos_cathay_data = dbGetQuery(connection, "
...
# str(cr_pagos_cathay_data)
cr_pagos_cathay_data$FECHA_PAGO = as.Date(cr_pagos_cathay_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(cr_pagos_cathay_data)
# # Se tienen observaciones desde 2012-06 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

cr_pagos_cathay = cr_pagos_cathay_data %>% 
...
# # por columna
# apply(cr_pagos_cathay, 2, porcentaje_empty)
# # 2.89% de la data en la que no hubo recuperación.

# Se completa la primera columna
cr_pagos_cathay$INSTITUCION = replace_na(as.character(cr_pagos_cathay$INSTITUCION), "CATHAY")
# Se interpolan los valores perdidos
cr_pagos_cathay = cr_pagos_cathay %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# El primer registro es 2012-06. Decido empezar la serie en 2013-01-01 para disminuir los datos artificiales (interpolados) y dejar como última observación el mes anterior al actual completo.  
init_first_month = '2013-01-01'
...
# residuos_data = diff(cr_pagos_cathay$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.0000000000009509) < (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.0000002553) < (alfa = 0.05)].
# 
...
serie = ts(cr_pagos_cathay$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -3.8708 y un p-value de = 0.01768. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 9.0 14.4 48.0
# # Es así como de determina que los períodos más importantes son cada 9.0 meses, cada 14.4 meses y cada 48.0 meses.
# 
...
# pred_arima = predict(model_arima, n.ahead = serie_training)
# pred_arima
# 
# # Arima calibrado
...
# model_calibrated_arima = arima(serie_training, order = c(0, 2, 0), seasonal = list(order = c(1, 0, 1), period = 12))
...
final_date = cr_pagos_cathay$FECHA_PAGO[nrow(cr_pagos_cathay)]
final_date
...
total_dates = c(cr_pagos_cathay$FECHA_PAGO, dates)
...
predicciones_finales_cathay = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_cathay) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_cathay = predicciones_finales_cathay %>%
...
saveRDS(predicciones_finales_cathay, "predicciones_finales_cathay.rds")
