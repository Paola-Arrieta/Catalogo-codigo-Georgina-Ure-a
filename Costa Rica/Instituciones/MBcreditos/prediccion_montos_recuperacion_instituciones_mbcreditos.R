# ---
# title: "Predicción de montos recuperación MB Créditos"
...
cr_pagos_mbcreditos_data = dbGetQuery(connection, "
...
# str(cr_pagos_mbcreditos_data)
cr_pagos_mbcreditos_data$FECHA_PAGO = as.Date(cr_pagos_mbcreditos_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(cr_pagos_mbcreditos_data)
# # Se tienen observaciones desde 2012-09-01 hasta 2023-09-01 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

cr_pagos_mbcreditos = cr_pagos_mbcreditos_data %>% 
...
# por columna
# apply(cr_pagos_mbcreditos, 2, porcentaje_empty)
# 0% de la data en la que no hubo recuperación (data completa).

# Se completa la primera columna
cr_pagos_mbcreditos$INSTITUCION = replace_na(as.character(cr_pagos_mbcreditos$INSTITUCION), "MB CREDITOS")
# Se interpolan los valores perdidos
cr_pagos_mbcreditos = cr_pagos_mbcreditos %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# El primer registro es	2021-03-01; empiezo la serie en el segundo mes de registro para mayor consistencia del dataset. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = '2021-04-01'
...
# residuos_data = diff(cr_pagos_mbcreditos$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.2359) > (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.1307) > (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.04015) < (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(cr_pagos_mbcreditos$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -2.19 y un p-value de = 0.4996. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 16.000000  5.333333  4.571429
# # Es así como de determina que los períodos más importantes son cada 16 meses (poco más del año), cada 5.3 meses (cada casi medio año) y cada 4.5 meses.
# 
...
# model_calibrated_arima = arima(serie_training, order = c(5, 3, 2), seasonal = list(order = c(0, 0, 0), period = 12))
...
final_date = cr_pagos_mbcreditos$FECHA_PAGO[nrow(cr_pagos_mbcreditos)]
final_date
...
total_dates = c(cr_pagos_mbcreditos$FECHA_PAGO, dates)
...
#   dyAxis("y", valueRange = c(0, 80000000))
...
predicciones_finales_mbcreditos = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_mbcreditos) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_mbcreditos = predicciones_finales_mbcreditos %>%
...
saveRDS(predicciones_finales_mbcreditos, "predicciones_finales_mbcreditos.rds")
