# ---
# title: "Predicción de montos recuperación Multivalores"
...
# summary(cr_pagos_multivalores_data)
# # Se tienen observaciones desde 2013-07-01 hasta 2023-10-01 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).
...
# # por columna
# apply(cr_pagos_multivalores, 2, porcentaje_empty)
...
# residuos_data = diff(cr_pagos_multivalores$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.0000003347) < (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.0002218) < (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.0000005655) < (alfa = 0.05)].
...
# # Se obtiene un valor de -3.3267 y un p-value de = 0.07006. La hipótesis nula no es aceptada y, por lo tanto, la serie es estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 2.232143 4.464286 3.787879
# # Es así como de determina que los períodos más importantes son cada 2.23 meses, cada 4.46 meses y cada 3.78 meses.
# 
