# ---
# title: "Predicción de montos recuperación Lafise"
...
cr_pagos_lafise_data = dbGetQuery(connection, "
...
# str(cr_pagos_lafise_data)
cr_pagos_lafise_data$FECHA_PAGO = as.Date(cr_pagos_lafise_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(cr_pagos_lafise_data)
# # Se tienen observaciones desde 2011-04-01 hasta 2023-10-01 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

cr_pagos_lafise = cr_pagos_lafise_data %>% 
...
# # por columna
# apply(cr_pagos_lafise, 2, porcentaje_empty)
# # 0% de la data en la que no hubo recuperación (data completa).

# Se completa la primera columna
cr_pagos_lafise$INSTITUCION = replace_na(as.character(cr_pagos_lafise$INSTITUCION), "LAFISE")
# Se interpolan los valores perdidos
cr_pagos_lafise = cr_pagos_lafise %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# El primer registro es 2011-04-01, pero, por consistencia de la data, empiezo la serie en 2011-06-01. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = '2011-06-01'
...
# residuos_data = diff(cr_pagos_lafise$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.00000429) < (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.001267) < (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.00006868) < (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(cr_pagos_lafise$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -4.0243 y un p-value de = 0.01. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 50.000000 75.000000  8.333333
# # Es así como de determina que los períodos más importantes son cada 50 meses (cada 4 años), cada 75 meses (poco más de cada 6 años) y cada 8.33 meses (cada poco más de medio año).
# 
...
# model_calibrated_arima = arima(serie_training, order = c(4, 5, 1), seasonal = list(order = c(1, 0, 1), period = 12))
...
final_date = cr_pagos_lafise$FECHA_PAGO[nrow(cr_pagos_lafise)]
final_date
...
total_dates = c(cr_pagos_lafise$FECHA_PAGO, dates)
...
#   dyAxis("y", valueRange = c(0, 9000000))
...
predicciones_finales_lafise = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_lafise) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_lafise = predicciones_finales_lafise %>%
...
saveRDS(predicciones_finales_lafise, "predicciones_finales_lafise.rds")
