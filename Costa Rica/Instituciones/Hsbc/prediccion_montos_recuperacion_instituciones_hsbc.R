# ---
# title: "Predicción de montos recuperación HSBC"
...
cr_pagos_hsbc_data = dbGetQuery(connection, "
...
# str(cr_pagos_hsbc_data)
cr_pagos_hsbc_data$FECHA_PAGO = as.Date(cr_pagos_hsbc_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(cr_pagos_hsbc_data)
# # Se tienen observaciones desde 2009-11-01 hasta 2023-09-01 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

cr_pagos_hsbc = cr_pagos_hsbc_data %>% 
...
# # por columna
# apply(cr_pagos_hsbc, 2, porcentaje_empty)
# # 0% de la data en la que no hubo recuperación (data completa).

# Se completa la primera columna
cr_pagos_hsbc$INSTITUCION = replace_na(as.character(cr_pagos_hsbc$INSTITUCION), "B-HSBC")
# Se interpolan los valores perdidos
cr_pagos_hsbc = cr_pagos_hsbc %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# El primer registro es 2009-11-01; por consistencia de la data, empiezo la serie el 2010-02-01. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = '2010-02-01'
...
# residuos_data = diff(cr_pagos_hsbc$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.007221) < (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.00005203) < (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.000007263) < (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(cr_pagos_hsbc$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -3.3986 y un p-value de = 0.03465. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 45 60 30
# # Es así como de determina que los períodos más importantes son cada 45 meses, cada 60 meses y cada 30 meses.
# 
...
# model_arima = arima(serie_training, order = c(0,1,3), seasonal = list(order = c(0,0,1), period = 12))
# model_arima
# 
...
# model_calibrated_arima = arima(serie_training, order = c(3, 2, 2), seasonal = list(order = c(0, 0, 0), period = 12))
...
final_date = cr_pagos_hsbc$FECHA_PAGO[nrow(cr_pagos_hsbc)]
final_date
...
total_dates = c(cr_pagos_hsbc$FECHA_PAGO, dates)
...
#   dyAxis("y", valueRange = c(0, 110000000))
# 
...
predicciones_finales_hsbc = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_hsbc) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_hsbc = predicciones_finales_hsbc %>%
...
saveRDS(predicciones_finales_hsbc, "predicciones_finales_hsbc.rds")
