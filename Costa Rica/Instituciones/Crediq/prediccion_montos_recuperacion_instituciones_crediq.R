# ---
# title: "Predicción de montos recuperación Crediq"
...
cr_pagos_crediq_data = dbGetQuery(connection, "
...
# str(cr_pagos_crediq_data)
cr_pagos_crediq_data$FECHA_PAGO = as.Date(cr_pagos_crediq_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(cr_pagos_crediq_data)
...
cr_pagos_crediq = cr_pagos_crediq_data %>% 
...
# # por columna
# apply(cr_pagos_crediq, 2, porcentaje_empty)
# # 0% de la data en la que no hubo recuperación (data completa).

# Se completa la primera columna
cr_pagos_crediq$INSTITUCION = replace_na(as.character(cr_pagos_crediq$INSTITUCION), "CREDIQ")
# Se interpolan los valores perdidos
cr_pagos_crediq = cr_pagos_crediq %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# El primer registro es 2017-01-01; por estabilidad de los datos, se decide empezar la serie en el mes siguiente. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = '2017-02-01'
...
# residuos_data = diff(cr_pagos_crediq$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.4281) > (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.3405) > (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.2048) > (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(cr_pagos_crediq$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -2.3823 y un p-value de = 0.4192. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 11.57143 27.00000 20.25000
# # Es así como de determina que los períodos más importantes son cada 11 meses (cada casi año), cada casi 27 meses (cada poco más de 2 años) y cada 20.25 meses (cada poco más de dos años).
# 
...
# model_calibrated_arima = arima(serie_training, order = c(0, 3, 2), seasonal = list(order = c(1, 0, 0), period = 12))
...
final_date = cr_pagos_crediq$FECHA_PAGO[nrow(cr_pagos_crediq)]
final_date
...
total_dates = c(cr_pagos_crediq$FECHA_PAGO, dates)
...
#   dyAxis("y", valueRange = c(0, 100000000))
...
predicciones_finales_crediq = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_crediq) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_crediq = predicciones_finales_crediq %>%
...
saveRDS(predicciones_finales_crediq, "predicciones_finales_crediq.rds")
