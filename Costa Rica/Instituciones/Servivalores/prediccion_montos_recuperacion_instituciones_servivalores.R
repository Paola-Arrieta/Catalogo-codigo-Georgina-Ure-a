# ---
# title: "Predicción de montos recuperación Servivalores"
...
cr_pagos_servivalores_data = dbGetQuery(connection, "
...
# str(cr_pagos_servivalores_data)
cr_pagos_servivalores_data$FECHA_PAGO = as.Date(cr_pagos_servivalores_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(cr_pagos_servivalores_data)
# # Se tienen observaciones desde 2000-03-01 hasta 2023-10-01 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

cr_pagos_servivalores = cr_pagos_servivalores_data %>% 
...
# por columna
# apply(cr_pagos_servivalores, 2, porcentaje_empty)
# 20.4 de la data en la que no hubo recuperación.

# Se completa la primera columna
cr_pagos_servivalores$INSTITUCION = replace_na(as.character(cr_pagos_servivalores$INSTITUCION), "SERVIVALORES")
# Se interpolan los valores perdidos
cr_pagos_servivalores = cr_pagos_servivalores %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# El primer registro es 2000-03-01. Decido acotar la data y dejar como primera observación el 2009-01-01, ya que no existen NA's a partir de este punto y no sería necesario recurrir a la imputación, y la data se estabaliza más. Además, elijo como última observación el mes anterior al actual completo.   
...
# residuos_data = diff(cr_pagos_servivalores$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.1187) > (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.01137) < (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.00777) < (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(cr_pagos_servivalores$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# Se obtiene un valor de -1.6658 y un p-value de 0.7159. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 96.0 64.0 38.4
# # Es así como de determina que los períodos más importantes son cada 96 meses (cada 8 años), cada 64 meses (poco más de cada 6 años) y cada 38.4 meses (cada poco más de 3 años).
# 
...
# model_calibrated_arima = arima(serie_training, order = c(0, 5, 5), seasonal = list(order = c(1, 1, 0), period = 12))
...
final_date = cr_pagos_servivalores$FECHA_PAGO[nrow(cr_pagos_servivalores)]
final_date
...
total_dates = c(cr_pagos_servivalores$FECHA_PAGO, dates)
...
#   dyAxis("y", valueRange = c(0, 125000000))
...
predicciones_finales_servivalores = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_servivalores) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_servivalores = predicciones_finales_servivalores %>%
...
saveRDS(predicciones_finales_servivalores, "predicciones_finales_servivalores.rds")
