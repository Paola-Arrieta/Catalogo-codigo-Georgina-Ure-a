# ---
# title: "Predicción de montos recuperación Cafsa"
...
cr_pagos_cafsa_data = dbGetQuery(connection, "
...
# str(cr_pagos_cafsa_data)
cr_pagos_cafsa_data$FECHA_PAGO = as.Date(cr_pagos_cafsa_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(cr_pagos_cafsa_data)
# # Se tienen observaciones desde 2015-05 hasta el año 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

cr_pagos_cafsa = cr_pagos_cafsa_data %>% 
...
# # por columna
# apply(cr_pagos_cafsa, 2, porcentaje_empty)
# # 10,78% de la data en la que no hubo recuperación.

# Se completa la primera columna
cr_pagos_cafsa$INSTITUCION = replace_na(as.character(cr_pagos_cafsa$INSTITUCION), "CAFSA")
# Se interpolan los valores perdidos
cr_pagos_cafsa = cr_pagos_cafsa %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# El primer registro es 2015-05-01; sin embargo, decido empezar la serie en 2017-06-01 porque es cuando los datos muestran mayor consistencia y son datos reales (no interpolados). Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = '2017-06-01'
...
# residuos_data = diff(cr_pagos_cafsa$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.0000000002727) < (alfa = 0.05)].
# 
...
serie = ts(cr_pagos_cafsa$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)

# # Gráfico de series de tiempo
# autoplot(serie, main = "Montos totales recuperados", xlab = "Ano", ylab = "Monto recuperado")
...
# # Se obtiene un valor de -5.1777 y un p-value de 0.01. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 3.724138 7.200000 4.000000
# # Es así como de determina que los períodos más importantes son cada 3.7 meses, cada 7.2 meses y cada 4 meses.
# 
...
# model_calibrated_arima = arima(serie_training, order = c(0, 2, 2), seasonal = list(order = c(0, 0, 1), period = 12))
...
final_date = cr_pagos_cafsa$FECHA_PAGO[nrow(cr_pagos_cafsa)]
final_date
...
total_dates = c(cr_pagos_cafsa$FECHA_PAGO, dates)
...
#   dygraphs::dyAxis("y", valueRange = c(0, 50000000))
...
predicciones_finales_cafsa = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_cafsa) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_cafsa = predicciones_finales_cafsa %>%
...
saveRDS(predicciones_finales_cafsa, "predicciones_finales_cafsa.rds")
