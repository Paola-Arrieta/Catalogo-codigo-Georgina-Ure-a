...
cr_pagos_citi_data = dbGetQuery(connection, "
...
# str(cr_pagos_citi_data)
cr_pagos_citi_data$FECHA_PAGO = as.Date(cr_pagos_citi_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(cr_pagos_citi_data)
# # Se tienen observaciones desde 2001-11 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

cr_pagos_citi = cr_pagos_citi_data %>% 
...
# # por columna
# apply(cr_pagos_citi, 2, porcentaje_empty)
# # 11.32% de la data en la que no hubo recuperación.

# Se completa la primera columna
cr_pagos_citi$INSTITUCION = replace_na(as.character(cr_pagos_citi$INSTITUCION), "CITI")
# Se interpolan los valores perdidos
cr_pagos_citi = cr_pagos_citi %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# Se decide dejar como primer registro 2016-01-01, ya que es cuando los montos presentan mayor estabilidad. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = '2016-01-01'
...
# residuos_data = diff(cr_pagos_citi$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.0703) > (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.1228) > (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.01628) < (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(cr_pagos_citi$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -5.5527 y un p-value de = 0.01. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 17.142857 40.000000  7.058824
# # Es así como de determina que los períodos más importantes son cada 17.14 meses (casi año y medio), cada 40 meses (cada poco más de tres años) y cada 7.05 meses (poco más de cada semestre).
# 
...
# model_calibrated_arima = arima(serie_training, order = c(3, 3, 1), seasonal = list(order = c(1, 1, 0), period = 12))
...
final_date = cr_pagos_citi$FECHA_PAGO[nrow(cr_pagos_citi)]
final_date
...
total_dates = c(cr_pagos_citi$FECHA_PAGO, dates)
...
#   dyAxis("y", valueRange = c(0, 120000000))
