...
cr_pagos_scotiabank_data = dbGetQuery(connection, "
...
# str(cr_pagos_scotiabank_data)
cr_pagos_scotiabank_data$FECHA_PAGO = as.Date(cr_pagos_scotiabank_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(cr_pagos_scotiabank_data)
# # Se tienen observaciones desde 2004-07-01 hasta 2023-10-01 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

cr_pagos_scotiabank = cr_pagos_scotiabank_data %>% 
...
# # por columna
# apply(cr_pagos_scotiabank, 2, porcentaje_empty)
# # 9.48% de la data en la que no hubo recuperación.

# Se completa la primera columna
cr_pagos_scotiabank$INSTITUCION = replace_na(as.character(cr_pagos_scotiabank$INSTITUCION), "SCOTIABANK")
# Se interpolan los valores perdidos
cr_pagos_scotiabank = cr_pagos_scotiabank %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# El primer registro del histórico es 2004-07-01. Decido acotar la data y dejar como primera observación el 2011-01-01, ya que no existen NA's a partir de este punto y no sería necesario recurrir a la imputación. Además, elijo como última observación el mes anterior al actual completo.  
init_first_month = '2011-01-01'
...
# residuos_data = diff(cr_pagos_scotiabank$MONTO_TOTAL)
# 
...
serie = ts(cr_pagos_scotiabank$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -3.0039 y un p-value de = 0.158. La hipótesis nula no es aceptada y, por lo tanto, la serie es estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 40 80 32
# # Es así como de determina que los períodos más importantes son cada 40 meses (cada poco más de 3 años), cada 80 meses (poco más de cada 6 años y medio) y cada 32 meses (cada poco más de 3 años y medio).
# 
...
final_date = cr_pagos_scotiabank$FECHA_PAGO[nrow(cr_pagos_scotiabank)]
final_date
...
total_dates = c(cr_pagos_scotiabank$FECHA_PAGO, dates)
...
#   dyAxis("y", valueRange = c(0, 450000000))
