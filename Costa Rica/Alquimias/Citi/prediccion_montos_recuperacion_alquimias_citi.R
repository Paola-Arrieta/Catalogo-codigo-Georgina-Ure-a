# ---
# title: "Predicción de montos recuperación: Alquimias de Citi"
...
cr_pagos_citi_alquimias_data = dbGetQuery(connection, "
...
# str(cr_pagos_citi_alquimias_data)
cr_pagos_citi_alquimias_data$FECHA_PAGO = as.Date(cr_pagos_citi_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(cr_pagos_citi_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
cr_pagos_citi_alquimias = cr_pagos_citi_alquimias_data %>% 
...
# n_alquimias = cr_pagos_citi_alquimias %>%
#   group_by(ALQUIMIA) %>%
#   tally() # Counts how many elements are in each group
# # Hay 2 alquimias; además, se debe determinar si las observaciones están en fechas consecutivas.
...
alquimias_mes_anterior = cr_pagos_citi_alquimias %>%
  select(-INSTITUCION) %>%
...
# # 1 alquimia con 36 fechas vacías.
...
fechas_prediccion = alquimias_interpoladas %>%
  ...
  dplyr::rename(ALQUIMIA_FECHAS = ALQUIMIA) %>%
  unnest(Fecha) 
...
predicciones_finales_citi_alquimias = bind_rows(alquimias_mes_anterior, predicciones_finales) %>%
  arrange(ALQUIMIA) %>%
  unite(Fecha_prueba, c(FECHA_PAGO, Fecha), na.rm = TRUE) %>%
  dplyr::rename(Alquimia=ALQUIMIA, Fecha=Fecha_prueba, Monto_real=MONTO_TOTAL) %>%
  as.data.frame() %>%
...
saveRDS(predicciones_finales_citi_alquimias, "predicciones_finales_citi_alquimias.rds")
