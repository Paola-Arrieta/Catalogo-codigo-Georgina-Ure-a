...
cr_pagos_scotiabank_alquimias_data = dbGetQuery(connection, "
...
# str(cr_pagos_scotiabank_alquimias_data)
cr_pagos_scotiabank_alquimias_data$FECHA_PAGO = as.Date(cr_pagos_scotiabank_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(cr_pagos_scotiabank_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
cr_pagos_scotiabank_alquimias = cr_pagos_scotiabank_alquimias_data %>% 
...
# n_alquimias = cr_pagos_scotiabank_alquimias %>%
#   group_by(ALQUIMIA) %>%
#   tally() # Counts how many elements are in each group
# # Hay 8 alquimias; además, se debe determinar si las observaciones están en fechas consecutivas.
...
alquimias_mes_anterior = cr_pagos_scotiabank_alquimias %>%
  select(-INSTITUCION) %>%
...
# # 3 alquimias con más de 40 fechas vacías.
