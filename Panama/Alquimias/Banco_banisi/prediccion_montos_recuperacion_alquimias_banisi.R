# ---
# title: "Predicción de montos recuperación: Alquimias de Banco Banisi"
...
pan_pagos_banisi_alquimias_data = dbGetQuery(connection, "
...
# str(pan_pagos_banisi_alquimias_data)
pan_pagos_banisi_alquimias_data$FECHA_PAGO = as.Date(pan_pagos_banisi_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(pan_pagos_banisi_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
pan_pagos_banisi_alquimias = pan_pagos_banisi_alquimias_data %>% 
...
# n_alquimias = pan_pagos_banisi_alquimias %>%
#   group_by(ALQUIMIA) %>%
...
alquimias_mes_anterior = pan_pagos_banisi_alquimias %>%
  select(-INSTITUCION) %>%
...
# # Ninguna alquimia con una cantidad considerable de fechas vacías (el número más alto es de 4 NA's).
...
predicciones_finales_banisi_alquimias = bind_rows(alquimias_mes_anterior, predicciones_finales) %>%
  arrange(ALQUIMIA) %>%
...
saveRDS(predicciones_finales_banisi_alquimias, "predicciones_finales_banisi_alquimias.rds")
