# ---
# title: "Predicción de montos recuperación: Alquimias de MULTIBANK"
...
pan_pagos_multibank_alquimias_data = dbGetQuery(connection, "
...
# str(pan_pagos_multibank_alquimias_data)
pan_pagos_multibank_alquimias_data$FECHA_PAGO = as.Date(pan_pagos_multibank_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(pan_pagos_multibank_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
pan_pagos_multibank_alquimias = pan_pagos_multibank_alquimias_data %>% 
...
# n_alquimias = pan_pagos_multibank_alquimias %>%
#   group_by(ALQUIMIA) %>%
#   tally() # Counts how many elements are in each group
# # Hay solamente 1 alquimia; además, se debe determinar si las observaciones están en fechas consecutivas.
...
alquimias_mes_anterior = pan_pagos_multibank_alquimias %>%
  select(-INSTITUCION) %>%
