...
pan_pagos_scotiabank_alquimias_data = dbGetQuery(connection, "
...
# str(pan_pagos_scotiabank_alquimias_data)
pan_pagos_scotiabank_alquimias_data$FECHA_PAGO = as.Date(pan_pagos_scotiabank_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(pan_pagos_scotiabank_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
pan_pagos_scotiabank_alquimias = pan_pagos_scotiabank_alquimias_data %>% 
...
# n_alquimias = pan_pagos_scotiabank_alquimias %>%
#   group_by(ALQUIMIA) %>%
...
alquimias_mes_anterior = pan_pagos_scotiabank_alquimias %>%
  select(-INSTITUCION) %>%
...
# # Ninguna alquimia con fechas vacías.
