# ---
# title: "Predicción de montos recuperación Banistmo"
...
pan_pagos_banistmo_data = dbGetQuery(connection, "
...
# str(pan_pagos_banistmo_data)
pan_pagos_banistmo_data$FECHA_PAGO = as.Date(pan_pagos_banistmo_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(pan_pagos_banistmo_data)
# # Se tienen observaciones desde 2019-12 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

pan_pagos_banistmo = pan_pagos_banistmo_data %>% 
...
# # por columna
# apply(pan_pagos_banistmo, 2, porcentaje_empty)
# 0% de la data en la que no hubo recuperación.

# Se completa la primera columna
pan_pagos_banistmo$INSTITUCION = replace_na(as.character(pan_pagos_banistmo$INSTITUCION), "BANISTMO")
# Se interpolan los valores perdidos
pan_pagos_banistmo = pan_pagos_banistmo %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# Se decide dejar como primer registro 2020-01-01, ya que la primera observación es un valor base. Decido dejar como última observación el mes anterior al actual completo.  
...
# residuos_data = diff(pan_pagos_banistmo$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.1224) > (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.06103) > (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.02675) < (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(pan_pagos_banistmo$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -2.8124 y un p-value de = 0.2503. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 9.6 24.0 16.0
# # Es así como de determina que los períodos más importantes son cada 9.6 meses (menos de un año), cada 24.00 meses (cada dos años) y cada 16.00 meses (cada año y tres meses).
# 
...
# model_calibrated_arima = arima(serie_training, order = c(0, 3, 1), seasonal = list(order = c(0, 1, 1), period = 12))
...
final_date = pan_pagos_banistmo$FECHA_PAGO[nrow(pan_pagos_banistmo)]
final_date
...
total_dates = c(pan_pagos_banistmo$FECHA_PAGO, dates)
...
predicciones_finales_banistmo = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_banistmo) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_banistmo = predicciones_finales_banistmo %>%
...
saveRDS(predicciones_finales_banistmo, "predicciones_finales_banistmo.rds")
