...
pan_pagos_citi_data = dbGetQuery(connection, "
...
# str(pan_pagos_citi_data)
pan_pagos_citi_data$FECHA_PAGO = as.Date(pan_pagos_citi_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(pan_pagos_citi_data)
...
pan_pagos_citi = pan_pagos_citi_data %>% 
...
# # por columna
# apply(pan_pagos_citi, 2, porcentaje_empty)
# # 0.0 de la data en la que no hubo recuperación.

# Se completa la primera columna
pan_pagos_citi$INSTITUCION = replace_na(as.character(pan_pagos_citi$INSTITUCION), "CITI")
# Se interpolan los valores perdidos
pan_pagos_citi = pan_pagos_citi %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# Se decide dejar como primer registro 2014-09, ya que es la primera observación registrada y no hay necesidad de acotar la serie. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = pan_pagos_citi$FECHA_PAGO[1]
...
# residuos_data = diff(pan_pagos_citi$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) no proviene de una distribución normal [(p-value = 0.5948) > (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) no proviene de una distribución normal [(p-values = 0.06229) > (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) no proviene de una distribución normal [(p-values = 0.226) > (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(pan_pagos_citi$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de 0.226 y un p-value de = 0.3739. La hipótesis nula es aceptada y, por lo tanto, la serie no es estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 60 30 12
# # Es así como de determina que los períodos más importantes son cada 60 meses (cada cinco años), cada 30.00 meses (cada dos años y medio) y cada 12.0 meses (cada año).
# 
...
# model_arima = arima(serie_training, order = c(0,1,1), seasonal = list(order = c(1,0,0), period = 12))
# model_arima
# 
...
# model_calibrated_arima = arima(serie_training, order = c(1, 4, 0), seasonal = list(order = c(1, 1, 0), period = 12))
...
final_date = pan_pagos_citi$FECHA_PAGO[nrow(pan_pagos_citi)]
final_date
...
total_dates = c(pan_pagos_citi$FECHA_PAGO, dates)
