...
pan_pagos_promerica_data = dbGetQuery(connection, "
...
# str(pan_pagos_promerica_data)
pan_pagos_promerica_data$FECHA_PAGO = as.Date(pan_pagos_promerica_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(pan_pagos_promerica_data)
# # Se tienen observaciones desde 2006-06 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

pan_pagos_promerica = pan_pagos_promerica_data %>% 
...
# # por columna
# apply(pan_pagos_promerica, 2, porcentaje_empty)
# # 0.95% de la data en la que no hubo recuperación.

# Se completa la primera columna
pan_pagos_promerica$INSTITUCION = replace_na(as.character(pan_pagos_promerica$INSTITUCION), "A-PROMERICA")
# Se interpolan los valores perdidos
pan_pagos_promerica = pan_pagos_promerica %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# Se decide dejar como primer registro 2008-04-01. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = '2008-04-01'
...
# residuos_data = diff(pan_pagos_promerica$MONTO_TOTAL)
# 
...
serie = ts(pan_pagos_promerica$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -3.9133 y un p-value de = 0.01449. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 108  72  54
# # Es así como de determina que los períodos más importantes son cada 108 meses (cada 9 meses), cada 72 meses (cada 6 años) y cada 54 meses (cada 4 años y medio).
# 
...
# model_arima = arima(serie_training, order = c(0,1,1), seasonal = list(order = c(0,0,1), period = 12))
# model_arima
# 
...
# model_calibrated_arima = arima(serie_training, order = c(0, 3, 0), seasonal = list(order = c(1, 1, 1), period = 12))
...
final_date = pan_pagos_promerica$FECHA_PAGO[nrow(pan_pagos_promerica)]
final_date
...
total_dates = c(pan_pagos_promerica$FECHA_PAGO, dates)
...
#   dyAxis("y", valueRange = c(0, 600000))
