# ---
# title: "Predicción de montos recuperación Crediviva"
...
pan_pagos_crediviva_data = dbGetQuery(connection, "
...
# str(pan_pagos_crediviva_data)
pan_pagos_crediviva_data$FECHA_PAGO = as.Date(pan_pagos_crediviva_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(pan_pagos_crediviva_data)
# # Se tienen observaciones desde 2021-10 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

pan_pagos_crediviva = pan_pagos_crediviva_data %>% 
...
# # por columna
# apply(pan_pagos_crediviva, 2, porcentaje_empty)
# # 0% de la data en la que no hubo recuperación.

# Se completa la primera columna
pan_pagos_crediviva$INSTITUCION = replace_na(as.character(pan_pagos_crediviva$INSTITUCION), "CREDIVIVA")
# Se interpolan los valores perdidos
pan_pagos_crediviva = pan_pagos_crediviva %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# Se decide dejar como primer registro la segunda fecha de registro 2021-11-01, ya que la primera es un valor atípico. Además, decido dejar como última observación el mes anterior al actual completo.  
init_first_month = '2021-11-01'
...
# residuos_data = diff(pan_pagos_crediviva$MONTO_TOTAL)
# 
...
# El histograma anterior presenta la distribución de los residuos para las observaciones. Se observa una distribución relativamente asimétrica negativa, lo que podría indicar que tal vez incluye valores atípicos.
# 
...
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.5494) > (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.9565) > (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.9204) > (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(pan_pagos_crediviva$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -3.6224 y un p-value de 0.0484. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 2.272727 3.571429 2.083333
# # Es así como de determina que los períodos más importantes son cada 2.27 meses (cada bimestre), cada 3.57 meses (casi cada cuatrimestre) y cada 2.08 meses (cada bimestre).
# 
...
# model_calibrated_arima = arima(serie_training, order = c(1, 2, 5), seasonal = list(order = c(0, 1, 1), period = 12))
...
final_date = pan_pagos_crediviva$FECHA_PAGO[nrow(pan_pagos_crediviva)]
final_date
...
total_dates = c(pan_pagos_crediviva$FECHA_PAGO, dates)

# Predicciones finales con dygraph
predictions = xts(xts(final_predictions, order.by = total_dates))

# dygraph(predictions, width = "100%", ylab = "Monto", main = "Monto de recuperacion para Banco crediviva - Panamá") %>%
...
predicciones_finales_crediviva = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_crediviva) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_crediviva = predicciones_finales_crediviva %>%
...
saveRDS(predicciones_finales_crediviva, "predicciones_finales_crediviva.rds")
