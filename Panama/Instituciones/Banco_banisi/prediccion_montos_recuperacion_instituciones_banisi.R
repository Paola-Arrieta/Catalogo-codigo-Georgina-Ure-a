# ---
# title: "Predicción de montos recuperación Banco Banisi"
...
pan_pagos_banisi_data = dbGetQuery(connection, "
...
# str(pan_pagos_banisi_data)
pan_pagos_banisi_data$FECHA_PAGO = as.Date(pan_pagos_banisi_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(pan_pagos_banisi_data)
# # Se tienen observaciones desde 2018-02 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

pan_pagos_banisi = pan_pagos_banisi_data %>% 
...
# # por columna
# apply(pan_pagos_banisi, 2, porcentaje_empty)
# # 0% de la data en la que no hubo recuperación.

# Se completa la primera columna
pan_pagos_banisi$INSTITUCION = replace_na(as.character(pan_pagos_banisi$INSTITUCION), "BANCO BANISI")
# Se interpolan los valores perdidos
pan_pagos_banisi = pan_pagos_banisi %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# Se decide dejar como primer registro la fecha 2018-08-01 para mayor consistencia de la data.Además, decido dejar como última observación el mes anterior al actual completo.  
...
# residuos_data = diff(pan_pagos_banisi$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.00000000000001405) < (alfa = 0.05)].
# 
...
serie = ts(pan_pagos_banisi$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -3.164 y un p-value de 0.1021. La hipótesis nula no es aceptada y, por lo tanto, la serie es estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 24.000000 12.000000  6.545455
# # Es así como de determina que los períodos más importantes son cada 24.0 meses (cada dos años), cada 12.0 meses (cada año) y cada 6.54 meses (cada semestre).
# 
...
# model_calibrated_arima = arima(serie_training, order = c(5, 0, 4), seasonal = list(order = c(0, 0, 0), period = 12))
...
final_date = pan_pagos_banisi$FECHA_PAGO[nrow(pan_pagos_banisi)]
final_date
...
total_dates = c(pan_pagos_banisi$FECHA_PAGO, dates)

# Predicciones finales con dygraph
predictions = xts(xts(final_predictions, order.by = total_dates))

# dygraph(predictions, width = "100%", ylab = "Monto", main = "Monto de recuperacion para Banco Banisi - Panamá") %>%
...
predicciones_finales_banisi = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_banisi) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_banisi = predicciones_finales_banisi %>%
...
saveRDS(predicciones_finales_banisi, "predicciones_finales_banisi.rds")
