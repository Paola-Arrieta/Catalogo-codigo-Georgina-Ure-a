...
pan_pagos_scotiabank_data = dbGetQuery(connection, "
...
# str(pan_pagos_scotiabank_data)
pan_pagos_scotiabank_data$FECHA_PAGO = as.Date(pan_pagos_scotiabank_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(pan_pagos_scotiabank_data)
...
pan_pagos_scotiabank = pan_pagos_scotiabank_data %>% 
...
# # por columna
# apply(pan_pagos_scotiabank, 2, porcentaje_empty)
# # 0.0% de la data en la que no hubo recuperación.

# Se completa la primera columna
pan_pagos_scotiabank$INSTITUCION = replace_na(as.character(pan_pagos_scotiabank$INSTITUCION), "SCOTIABANK")
# Se interpolan los valores perdidos
pan_pagos_scotiabank = pan_pagos_scotiabank %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# Se decide dejar como primer registro 2018-05, ya que el monto del primer mes es sumamente bajo, mientras que a partir del segundo se muestra mayor estabilidad. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = '2018-05-01'
...
# residuos_data = diff(pan_pagos_scotiabank$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.3423) > (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.1493) > (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.02909) < (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(pan_pagos_scotiabank$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -3.0639 y un p-value de 0.1432. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 24 18 36
# # Es así como de determina que los períodos más importantes son cada 24 meses (cada 2 años), cada 18 meses (cada año y medio) y cada 36 meses (cada 3 años).
# 
...
# model_arima = arima(serie_training, order = c(1,0,1), seasonal = list(order = c(0,0,0), period = 12))
# model_arima
# 
...
# model_calibrated_arima = arima(serie_training, order = c(4, 4, 1), seasonal = list(order = c(1, 1, 0), period = 12))
...
# 
# # Se confirma, nuevamente, que el mejor modelo es Holt-Winters calibrado porque muestra valores más bajos para MSE y RMSE, aunque todos los demás modelos (a excepción del ARIMA calibrado) se comportan muy parecido.
...
final_date = pan_pagos_scotiabank$FECHA_PAGO[nrow(pan_pagos_scotiabank)]
final_date
...
total_dates = c(pan_pagos_scotiabank$FECHA_PAGO, dates)
...
#   dyAxis("y", valueRange = c(0, 1000000))
