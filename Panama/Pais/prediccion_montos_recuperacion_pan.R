...
panama_pagos = dbGetQuery(connection, "
...
# str(panama_pagos)
panama_pagos$FECHA_PAGO = as.Date(panama_pagos$FECHA_PAGO,format="%d/%m/%Y")
# summary(panama_pagos)
# # Se tienen observaciones desde 2006-06 a 2023 (no equiespaciadas o consecutivas)

# panama_pagos = panama_pagos %>%
#   arrange(MONTO)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso). 
pan_pagos = panama_pagos %>% 
...
# por columna
# apply(pan_pagos, 2, porcentaje_empty)
# 0.95% de la data en la que no hubo recuperación.

# Se decide, entonces, empezar la serie desde el 2009-01-01, para mayor estabilidad de la data, y dejar como última observación el mes anterior al actual completo.  
...
# residuos_data = diff(pan_pagos$MONTO_TOTAL)
# 
...
# # El histograma anterior presenta la distribución de los residuos para las observaciones. Se observa una distribución relativamente simétrica, lo que podría indicar que podría no incluir valores atípicos.
# 
...
serie = ts(pan_pagos$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
# El objeto creado anteriormente inicia en 135.0000 como monto de pago por todos los clientes de Panamá y finaliza en  1698730 en octubre 2023. Es decir, es una sucesión ordenada de valores.
...
# # Se obtiene un valor de -1.5567 y un p-value de 0.7619. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.  
...
# periods
# # 43.2 108.0  36.0
# # Es así como de determina que los períodos más importantes son cada 54.0 meses (cada 4 años y medio), cada 21.6 meses (cada poco más e año y medio) y cada 36.0 meses (cada 3 años).
# 
...
# model_arima = arima(serie_training, order = c(0,1,5), seasonal = list(order = c(0,0,1), period = 12))
# model_arima
# 
# 
...
# model_calibrated_arima = arima(serie_training, order = c(0, 0, 5), seasonal = list(order = c(0, 0, 0), period = 12))
...
# 
# errors
# # El cuadro anterior detalla los índices de error para cada uno de los modelos planteados. De esta manera se confirma que Arima calibrado no es la mejor opción, ya que sus valores en MSE y RMSE son muy altos, mientras que Holt-Winters y Holt-Winters calibrado muestran lo contrario.
# 
...
final_date = pan_pagos$FECHA_PAGO[nrow(pan_pagos)]
final_date
...
total_dates = c(pan_pagos$FECHA_PAGO, dates)
...
#   dyAxis("y", valueRange = c(0,2500000))
# 
...
predicciones_finales_pan = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame() 
names(predicciones_finales_pan) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")
predicciones_finales_pan = predicciones_finales_pan %>%
...
saveRDS(predicciones_finales_pan, "predicciones_finales_pan.rds")
