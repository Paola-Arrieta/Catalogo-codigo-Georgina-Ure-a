...
guatemala_pagos = dbGetQuery(connection, "
...
# str(guatemala_pagos)
guatemala_pagos$FECHA_PAGO = as.Date(guatemala_pagos$FECHA_PAGO,format="%d/%m/%Y")
# summary(guatemala_pagos)
# # Se tienen observaciones desde 2006-10 a hoy (no equiespaciadas o consecutivas)

# guatemala_pagos = guatemala_pagos %>%
#   arrange(MONTO)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso). 
gtm_pagos = guatemala_pagos %>% 
...
# por columna
# apply(gtm_pagos, 2, porcentaje_empty)
# 12.13% de la data en la que no hubo recuperación.
# # Se podría trabajar desde 2009-01-01, ya que a partir de ahí la data está completa hasta la fecha actual.

# Se decide, entonces, empezar la serie en 2009-01-01 y dejar como última observación el mes anterior al actual completo.  
...
# residuos_data = diff(gtm_pagos$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.0000000003644) < (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.00000000319) < (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.0000000008859) < (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(gtm_pagos$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
# El objeto creado anteriormente inicia en 1175.69 como monto de pago por todos los clientes de Guatemala y finaliza en 5076682 en octubre 2023. Es decir, es una sucesión ordenada de valores.
...
# # Se obtiene un valor de -2.3187y un p-value de 0.4431. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.  
...
# periods
# # 36 45 90
# # Es así como de determina que los períodos más importantes son cada 67.5 meses (poco más de cada 5 años y medio), cada 90.0 meses (cada 7 años y medio) y cada 135.0 meses (poco más de cada 11 años).
# 
...
# model_calibrated_arima = arima(serie_training, order = c(0, 0, 0), seasonal = list(order = c(0, 0, 0), period = 12))
...
# 
# # Aunque se confirma, nuevamente, que el mejor modelo es Holt-Winters calibrado porque muestra valores más bajos para MSE y RMSE, cabe destacar que se asemeja mucho al modelo Holt-Winters.
...
final_date = gtm_pagos$FECHA_PAGO[nrow(gtm_pagos)]
final_date
...
total_dates = c(gtm_pagos$FECHA_PAGO, dates)

# Predicciones finales con dygraph
predictions = xts(xts(final_predictions, order.by = total_dates))

# dygraph(predictions, width = "100%", ylab = "Monto", main = "Monto de recuperacion para Guatemala") %>%
...
predicciones_finales_gtm= cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_gtm) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")
predicciones_finales_gtm = predicciones_finales_gtm %>%
...
saveRDS(predicciones_finales_gtm, "predicciones_finales_gtm.rds")
