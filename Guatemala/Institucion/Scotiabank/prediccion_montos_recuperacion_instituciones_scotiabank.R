...
# str(gtm_pagos_scotiabank_data)
gtm_pagos_scotiabank_data$FECHA_PAGO = as.Date(gtm_pagos_scotiabank_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(gtm_pagos_scotiabank_data)
# # Se tienen observaciones desde 2011-11 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

gtm_pagos_scotiabank = gtm_pagos_scotiabank_data %>% 
...
# # por columna
# apply(gtm_pagos_scotiabank, 2, porcentaje_empty)
# # 0.0% de la data en la que no hubo recuperación.

# Se completa la primera columna
gtm_pagos_scotiabank$INSTITUCION = replace_na(as.character(gtm_pagos_scotiabank$INSTITUCION), "SCOTIABANK")
# Se interpolan los valores perdidos
gtm_pagos_scotiabank = gtm_pagos_scotiabank %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# Se decide dejar como primer registro 2011-11-10, pero, por estabilidad de la data, empiezo la serie un mes después. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = '2011-12-01'
...
# residuos_data = diff(gtm_pagos_scotiabank$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.03976) < (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# nortest::lillie.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.5863) > (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# nortest::cvm.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.5471) > (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(gtm_pagos_scotiabank$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -1.9957 y un p-value de 0.5782. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 36 48 72
# # Es así como de determina que los períodos más importantes son cada 36 meses (cada tres años), cada 48 meses (cada cuatro años) y cada 72 meses (cada seis años).
# 
...
# model_calibrated_arima = arima(serie_training, order = c(0, 3, 1), seasonal = list(order = c(1, 1, 1), period = 12))
...
# 
# # Se confirma, nuevamente, que el mejor modelo es Holt-Winters calibrado porque muestra valores más bajos para MSE y RMSE. Aunque, cabe destacar que Holt-Winters se parece mucho a este último.
...
final_date = gtm_pagos_scotiabank$FECHA_PAGO[nrow(gtm_pagos_scotiabank)]
final_date
...
total_dates = c(gtm_pagos_scotiabank$FECHA_PAGO, dates)
