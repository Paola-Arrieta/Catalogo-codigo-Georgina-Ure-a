...
gtm_pagos_promerica_data = dbGetQuery(connection, "
...
# str(gtm_pagos_promerica_data)
gtm_pagos_promerica_data$FECHA_PAGO = as.Date(gtm_pagos_promerica_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(gtm_pagos_promerica_data)
# # Se tienen observaciones desde 2006-10 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

gtm_pagos_promerica = gtm_pagos_promerica_data %>% 
...
# # por columna
# apply(gtm_pagos_promerica, 2, porcentaje_empty)
# # 12.13% de la data en la que no hubo recuperación.

# Se completa la primera columna
gtm_pagos_promerica$INSTITUCION = replace_na(as.character(gtm_pagos_promerica$INSTITUCION), "A-PROMERICA")
# Se interpolan los valores perdidos
gtm_pagos_promerica = gtm_pagos_promerica %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# Se decide dejar como primer registro 2009-06-01, ya que es cuando los montos presentan mayor estabilidad. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = '2009-06-01'
...
# residuos_data = diff(gtm_pagos_promerica$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.009801) < (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# nortest::cvm.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.0002714) < (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(gtm_pagos_promerica$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -2.6363 y un p-value de 0.3105. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 90.00000 45.00000 25.71429
# # Es así como de determina que los períodos más importantes son cada 90.00 meses (cada siete años y medio), cada 45 meses (cada poco menos de cuatro años) y cada 25.71 meses (cada poco más de dos años).
# 
...
# model_arima = arima(serie_training, order = c(2,0,2), seasonal = list(order = c(0,0,0), period = 12))
# model_arima
# 
...
# model_calibrated_arima = arima(serie_training, order = c(0, 4, 5), seasonal = list(order = c(0, 1, 1), period = 12))
...
final_date = gtm_pagos_promerica$FECHA_PAGO[nrow(gtm_pagos_promerica)]
final_date
...
total_dates = c(gtm_pagos_promerica$FECHA_PAGO, dates)

# Predicciones finales con dygraph
predictions = xts(xts(final_predictions, order.by = total_dates))

# dygraphs::dygraph(predictions, width = "100%", ylab = "Monto", main = "Monto de recuperacion para Promérica - Guatemala") %>%
...
#   dygraphs::dyAxis("y", valueRange = c(0, 1500000))
