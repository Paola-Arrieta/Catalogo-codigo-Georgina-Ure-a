...
gtm_pagos_citi_data = dbGetQuery(connection, "
...
# str(gtm_pagos_citi_data)
gtm_pagos_citi_data$FECHA_PAGO = as.Date(gtm_pagos_citi_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(gtm_pagos_citi_data)
...
gtm_pagos_citi = gtm_pagos_citi_data %>% 
...
# # por columna
# apply(gtm_pagos_citi, 2, porcentaje_empty)
# # 0.0% de la data en la que no hubo recuperación.

# Se completa la primera columna
gtm_pagos_citi$INSTITUCION = replace_na(as.character(gtm_pagos_citi$INSTITUCION), "CITI")
# Se interpolan los valores perdidos
gtm_pagos_citi = gtm_pagos_citi %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# Se decide dejar como primer registro 2014-09-01. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = gtm_pagos_citi$FECHA_PAGO[1]
...
# residuos_data = diff(gtm_pagos_citi$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.003185) < (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# nortest::lillie.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.001857) < (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# nortest::cvm.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.00002195) < (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(gtm_pagos_citi$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -1.7058 y un p-value de 0.6987. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 40 30 24
# # Es así como de determina que los períodos más importantes son cada 40 meses (cada poco más de tres años), cada 30 meses (cada dos años y medio) y cada 24 meses (cada dos años).
# 
...
final_date = gtm_pagos_citi$FECHA_PAGO[nrow(gtm_pagos_citi)]
final_date
...
total_dates = c(gtm_pagos_citi$FECHA_PAGO, dates)
