...
gtm_pagos_promerica_alquimias_data = dbGetQuery(connection, "
...
# str(gtm_pagos_promerica_alquimias_data)
gtm_pagos_promerica_alquimias_data$FECHA_PAGO = as.Date(gtm_pagos_promerica_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(gtm_pagos_promerica_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
gtm_pagos_promerica_alquimias = gtm_pagos_promerica_alquimias_data %>% 
...
# n_alquimias = gtm_pagos_promerica_alquimias %>%
#   group_by(ALQUIMIA) %>%
#   tally() # Counts how many elements are in each group
# # Hay 118 alquimias; además, se debe determinar si las observaciones están en fechas consecutivas.
...
alquimias_mes_anterior = gtm_pagos_promerica_alquimias %>%
  select(-INSTITUCION) %>%
...
# # Hay 36 alquimias con más de 20 fechas vacías; inclusive, existen 20 con más de 40 casos en blanco.
