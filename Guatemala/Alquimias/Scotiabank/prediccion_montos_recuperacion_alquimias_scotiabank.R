# ---
# title: "Predicción de montos recuperación: Alquimias de Scotiabank"
...
gtm_pagos_scotiabank_alquimias_data = dbGetQuery(connection, "
...
# str(gtm_pagos_scotiabank_alquimias_data)
gtm_pagos_scotiabank_alquimias_data$FECHA_PAGO = as.Date(gtm_pagos_scotiabank_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(gtm_pagos_scotiabank_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
gtm_pagos_scotiabank_alquimias = gtm_pagos_scotiabank_alquimias_data %>% 
...
# n_alquimias = gtm_pagos_scotiabank_alquimias %>%
#   group_by(ALQUIMIA) %>%
#   tally() # Counts how many elements are in each group
# # Hay 75 alquimias; además, se debe determinar si las observaciones están en fechas consecutivas.
...
alquimias_mes_anterior = gtm_pagos_scotiabank_alquimias %>%
  select(-INSTITUCION) %>%
...
# # Hay solamente una alquimia con más de 20 fechas vacías, las demás alquimias están bastante completas.
...
predicciones_finales_scotiabank_alquimias = bind_rows(alquimias_mes_anterior, predicciones_finales) %>%
  arrange(ALQUIMIA) %>%
...
saveRDS(predicciones_finales_scotiabank_alquimias, "predicciones_finales_scotiabank_alquimias.rds")
