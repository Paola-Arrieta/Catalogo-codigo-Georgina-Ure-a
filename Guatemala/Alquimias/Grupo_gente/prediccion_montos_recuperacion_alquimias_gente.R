...
gtm_pagos_gente_alquimias_data = dbGetQuery(connection, "
...
# str(gtm_pagos_gente_alquimias_data)
gtm_pagos_gente_alquimias_data$FECHA_PAGO = as.Date(gtm_pagos_gente_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(gtm_pagos_gente_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
gtm_pagos_gente_alquimias = gtm_pagos_gente_alquimias_data %>% 
...
n_alquimias = gtm_pagos_gente_alquimias %>%
  group_by(ALQUIMIA) %>%
...
alquimias_mes_anterior = gtm_pagos_gente_alquimias %>%
  select(-INSTITUCION) %>%
