# ---
# title: "Predicción de montos recuperación: Alquimias de Ficohsa"
...
nic_pagos_ficohsa_alquimias_data = dbGetQuery(connection, "
...
# str(nic_pagos_ficohsa_alquimias_data)
nic_pagos_ficohsa_alquimias_data$FECHA_PAGO = as.Date(nic_pagos_ficohsa_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(nic_pagos_ficohsa_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
nic_pagos_ficohsa_alquimias = nic_pagos_ficohsa_alquimias_data %>% 
...
# n_alquimias = nic_pagos_ficohsa_alquimias %>%
#   group_by(ALQUIMIA) %>%
...
alquimias_mes_anterior = nic_pagos_ficohsa_alquimias %>%
  select(-INSTITUCION) %>%
...
predicciones_finales_ficohsa_alquimias = bind_rows(alquimias_mes_anterior, predicciones_finales) %>%
  arrange(ALQUIMIA) %>%
...
saveRDS(predicciones_finales_ficohsa_alquimias, "predicciones_finales_ficohsa_alquimias.rds")
