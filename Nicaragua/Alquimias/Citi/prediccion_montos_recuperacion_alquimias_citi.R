...
nic_pagos_citi_alquimias_data = dbGetQuery(connection, "
...
# str(nic_pagos_citi_alquimias_data)
nic_pagos_citi_alquimias_data$FECHA_PAGO = as.Date(nic_pagos_citi_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(nic_pagos_citi_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
nic_pagos_citi_alquimias = nic_pagos_citi_alquimias_data %>% 
...
# n_alquimias = nic_pagos_citi_alquimias %>%
#   group_by(ALQUIMIA) %>%
...
alquimias_mes_anterior = nic_pagos_citi_alquimias %>%
  select(-INSTITUCION) %>%
