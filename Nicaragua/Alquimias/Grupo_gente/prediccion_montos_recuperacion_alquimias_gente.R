# ---
# title: "Predicción de montos recuperación: Alquimias de Grupo Gente"
...
nic_pagos_gente_alquimias_data = dbGetQuery(connection, "
...
# str(nic_pagos_gente_alquimias_data)
nic_pagos_gente_alquimias_data$FECHA_PAGO = as.Date(nic_pagos_gente_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(nic_pagos_gente_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
nic_pagos_gente_alquimias = nic_pagos_gente_alquimias_data %>% 
...
# n_alquimias = nic_pagos_gente_alquimias %>%
#   group_by(ALQUIMIA) %>%
...
alquimias_mes_anterior = nic_pagos_gente_alquimias %>%
  select(-INSTITUCION) %>%
...
# Se necesitan algunos meses más para que las alquimias de la institución cumplan con el mínimo de observaciones para poder realizar un análisis de series de tiempo (24).