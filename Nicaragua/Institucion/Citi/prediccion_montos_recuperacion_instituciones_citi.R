# ---
# title: "Predicción de montos recuperación Citi"
...
nic_pagos_citi_data = dbGetQuery(connection, "
...
# str(nic_pagos_citi_data)
nic_pagos_citi_data$FECHA_PAGO = as.Date(nic_pagos_citi_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(nic_pagos_citi_data)
# # Se tienen observaciones desde 2014-09 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

nic_pagos_citi = nic_pagos_citi_data %>% 
...
# # por columna
# apply(nic_pagos_citi, 2, porcentaje_empty)
# # 0.0% de la data en la que no hubo recuperación.

# Se completa la primera columna
nic_pagos_citi$INSTITUCION = replace_na(as.character(nic_pagos_citi$INSTITUCION), "CITI")
# Se interpolan los valores perdidos
nic_pagos_citi = nic_pagos_citi %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# Se decide dejar como primer registro 2014-09-01; es decir, la primera fecha de registro de las observaciones. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = nic_pagos_citi$FECHA_PAGO[1]
...
# residuos_data = diff(nic_pagos_citi$MONTO_TOTAL)
# 
...
# # El histograma anterior presenta la distribución de los residuos para las observaciones. Se observa una distribución levemente asimétrica positiva, lo que podría indicar que tal vez incluye valores atípicos.
# 
...
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.03511) < (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# nortest::lillie.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.004003) < (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# nortest::cvm.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.007367) < (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(nic_pagos_citi$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -1.5467 y un p-value de 0.7647. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 60.0 15.0  7.5
# # Es así como de determina que los períodos más importantes son cada 60.0 meses (cada cinco años), cada 15.0 meses (cada poco más de un año) y cada 7.5 meses (cada poco más del semestre).
# 
...
# autoplot(serie_training, xlab = "Fecha", ylab = "", main = "Montos totales de recuperacion para Citi - Nicaragua") +
...
final_date = nic_pagos_citi$FECHA_PAGO[nrow(nic_pagos_citi)]
final_date
...
total_dates = c(nic_pagos_citi$FECHA_PAGO, dates)

# Predicciones finales con dygraph
predictions = xts(xts(final_predictions, order.by = total_dates))

# dygraphs::dygraph(predictions, width = "100%", ylab = "Monto", main = "Monto de recuperacion para Citi - Nicaragua") %>%
...
predicciones_finales_citi = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_citi) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_citi = predicciones_finales_citi %>%
...
saveRDS(predicciones_finales_citi, "predicciones_finales_citi.rds")
