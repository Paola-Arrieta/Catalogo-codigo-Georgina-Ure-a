# ---
# title: "Predicción de montos recuperación Santander"
...
mex_pagos_santander_data = dbGetQuery(connection, "
...
# str(mex_pagos_santander_data)
mex_pagos_santander_data$FECHA_PAGO = as.Date(mex_pagos_santander_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(mex_pagos_santander_data)
# # Se tienen observaciones desde 2022-10 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

mex_pagos_santander = mex_pagos_santander_data %>% 
