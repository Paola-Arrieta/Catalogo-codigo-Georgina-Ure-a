# ---
# title: "Predicción de montos recuperación Crédito Familiar"
...
mex_pagos_familiar_data = dbGetQuery(connection, "
                                 ...
                                 WHERE PAI_NOMBRE = 'MEXICO'
                                  ...
                                  AND INSTITUCION = 'CREDITO FAMILIAR' 
                                  ")
...
# str(mex_pagos_familiar_data)
mex_pagos_familiar_data$FECHA_PAGO = as.Date(mex_pagos_familiar_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(mex_pagos_familiar_data)
# # Se tienen observaciones desde 2014-01 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

mex_pagos_familiar = mex_pagos_familiar_data %>% 
...
# # por columna
# apply(mex_pagos_familiar, 2, porcentaje_empty)
# # 0.0% de la data en la que no hubo recuperación.

# Se completa la primera columna
mex_pagos_familiar$INSTITUCION = replace_na(as.character(mex_pagos_familiar$INSTITUCION), "familiar")
# Se interpolan los valores perdidos
mex_pagos_familiar = mex_pagos_familiar %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# Aunque el primer registro es 2014-01-01, se decide empezar la serie el 2020-01-01 debido a que es cuando la data muestra mayor estabilidad y consistencia en los montos recaudados. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = '2020-01-01'
...
# residuos_data = diff(mex_pagos_familiar$MONTO_TOTAL)
# 
...
# # El histograma anterior presenta la distribución de los residuos para las observaciones. Se observa una distribución medio simétrica, lo que podría indicar que tal vez no incluye valores atípicos.
# 
...
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.4061) > (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# nortest::lillie.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.4941) > (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# nortest::cvm.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.6153) > (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(mex_pagos_familiar$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -1.324 y un p-value de 0.8428. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 24.000000  2.400000  2.526316
# # Es así como de determina que los períodos más importantes son cada 24.0 meses (cada dos años), cada 2.4 meses (cada poco más del bimestre) y cada 2.52 meses (cada casi trimestre).
# 
...
# model_arima = arima(serie_training, order = c(1,1,0), seasonal = list(order = c(0,0,0), period = 12))
# model_arima
# 
...
# model_calibrated_arima = arima(serie_training, order = c(0, 2, 1), seasonal = list(order = c(0, 0, 1), period = 12))
...
# 
# # Se confirma, nuevamente, que el mejor modelo es Holt-Winters calibrado porque muestra valores más bajos para MSE, RMSE y PTFA. Aunque, cabe destacar que ARIMA se parece mucho a este último.
...
final_date = mex_pagos_familiar$FECHA_PAGO[nrow(mex_pagos_familiar)]
final_date
...
total_dates = c(mex_pagos_familiar$FECHA_PAGO, dates)

# Predicciones finales con dygraph
predictions = xts(xts(final_predictions, order.by = total_dates))

# dygraphs::dygraph(predictions, width = "100%", ylab = "Monto", main = "Monto de recuperacion para Costa Rica") %>%
...
predicciones_finales_familiar = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_familiar) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_familiar = predicciones_finales_familiar %>%
...
saveRDS(predicciones_finales_familiar, "predicciones_finales_familiar.rds")
