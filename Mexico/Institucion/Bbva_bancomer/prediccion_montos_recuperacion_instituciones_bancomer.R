# ---
# title: "Predicción de montos recuperación BBVA Bancomer"
...
mex_pagos_bancomer_data = dbGetQuery(connection, "
...
# str(mex_pagos_bancomer_data)
mex_pagos_bancomer_data$FECHA_PAGO = as.Date(mex_pagos_bancomer_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(mex_pagos_bancomer_data)
# # Se tienen observaciones desde 2017-01 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

mex_pagos_bancomer = mex_pagos_bancomer_data %>% 
...
# # por columna
# apply(mex_pagos_bancomer, 2, porcentaje_empty)
# # 0.0% de la data en la que no hubo recuperación.

# Se completa la primera columna
mex_pagos_bancomer$INSTITUCION = replace_na(as.character(mex_pagos_bancomer$INSTITUCION), "bancomer")
# Se interpolan los valores perdidos
mex_pagos_bancomer = mex_pagos_bancomer %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# Se decide dejar como primer registro 2019-01-01, debido a que es cuando la data muestra mayor estabilidad y consistencia en los montos recaudados. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = '2019-01-01'
...
# residuos_data = diff(mex_pagos_bancomer$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.7926) > (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# nortest::lillie.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.4233) > (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# nortest::cvm.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.3827) > (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(mex_pagos_bancomer$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -3.6149 y un p-value de 0.03805. La hipótesis nula no es aceptada y, por lo tanto, la serie es estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 15.0 37.5 25.0
# # Es así como de determina que los períodos más importantes son cada 15 meses (cada años y tres meses), cada 37.5 meses (cada poco más de tres años) y cada 25 meses (cada dos años).
# 
...
# model_calibrated_arima = arima(serie_training, order = c(0, 4, 1), seasonal = list(order = c(0, 0, 1), period = 12))
...
# autoplot(serie_training, xlab = "Fecha", ylab = "", main = "Montos totales de recuperacion para Bancomer") +
...
final_date = mex_pagos_bancomer$FECHA_PAGO[nrow(mex_pagos_bancomer)]
final_date
...
total_dates = c(mex_pagos_bancomer$FECHA_PAGO, dates)

# Predicciones finales con dygraph
predictions = xts(xts(final_predictions, order.by = total_dates))

# dygraphs::dygraph(predictions, width = "100%", ylab = "Monto", main = "Monto de recuperacion para Bancomer") %>%
...
#   dygraphs::dyAxis("y", valueRange = c(0, 10000000))
...
predicciones_finales_bancomer = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_bancomer) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_bancomer = predicciones_finales_bancomer %>%
...
saveRDS(predicciones_finales_bancomer, "predicciones_finales_bancomer.rds")
