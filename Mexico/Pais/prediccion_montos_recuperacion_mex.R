...
mexico_pagos = dbGetQuery(connection, "
...
# str(mexico_pagos)
mexico_pagos$FECHA_PAGO = as.Date(mexico_pagos$FECHA_PAGO,format="%d/%m/%Y")
# summary(mexico_pagos)
# # Se tienen observaciones desde 2012-06 a 2023 (no equiespaciadas o consecutivas)

# mexico_pagos = mexico_pagos %>%
#   arrange(MONTO)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso). 
mex_pagos = mexico_pagos %>% 
...
# por columna
# apply(mex_pagos, 2, porcentaje_empty)
# 2.89% de la data en la que no hubo recuperación.

# Se decide, entonces, empezar la serie desde en 2015-01-01 (para estabilidad y consistencia en la data) y dejar como última observación el mes anterior al actual completo.  
...
# residuos_data = diff(mex_pagos$MONTO_TOTAL)
# 
...
# # El histograma anterior presenta la distribución de los residuos para las observaciones. Se observa una distribución relativamente simétrica, tal vez un poco asimétrica negativa, lo que podría indicar que podría no incluir valores atípicos.
# 
...
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.004611) < (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.00332) < (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.0001814) < (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(mex_pagos$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
# El objeto creado anteriormente inicia en 118056.1 como monto de pago por todos los clientes de México y finaliza en  5704365 en octubre 2023. Es decir, es una sucesión ordenada de valores.
...
# # Se obtiene un valor de -1.4885 y un p-value de 0.7892. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.  
...
# model_calibrated_arima = arima(serie_training, order = c(4, 5, 0), seasonal = list(order = c(0, 0, 1), period = 12))
...
# 
# # Se confirma, nuevamente, que ARIMA calibrado no es el mejor modelo, mientras que ARIMA, Holt-Winters y Holt-Winters calibrado muestran índices muy similares tanto en MSE y RMSE, como en PTFA.
...
final_date = mex_pagos$FECHA_PAGO[nrow(mex_pagos)]
final_date
...
total_dates = c(mex_pagos$FECHA_PAGO, dates)
...
#   dyAxis("y", valueRange = c(0,10000000))
# 
...
predicciones_finales_mex = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame() 
names(predicciones_finales_mex) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")
predicciones_finales_mex = predicciones_finales_mex %>%
...
saveRDS(predicciones_finales_mex, "predicciones_finales_mex.rds")
