# ---
# title: "Predicción de montos recuperación: Alquimias de Daimler"
...
mex_pagos_daimler_alquimias_data = dbGetQuery(connection, "
...
# str(mex_pagos_daimler_alquimias_data)
mex_pagos_daimler_alquimias_data$FECHA_PAGO = as.Date(mex_pagos_daimler_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(mex_pagos_daimler_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
mex_pagos_daimler_alquimias = mex_pagos_daimler_alquimias_data %>% 
...
# n_alquimias = mex_pagos_daimler_alquimias %>%
#   group_by(ALQUIMIA) %>%
...
alquimias_mes_anterior = mex_pagos_daimler_alquimias %>%
  select(-INSTITUCION) %>%
