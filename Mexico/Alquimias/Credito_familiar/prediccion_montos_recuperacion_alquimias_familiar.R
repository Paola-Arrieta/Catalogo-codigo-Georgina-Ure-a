# ---
# title: "Predicción de montos recuperación: Alquimias de Crédito familiar"
...
mex_pagos_familiar_alquimias_data = dbGetQuery(connection, "
...
# str(mex_pagos_familiar_alquimias_data)
mex_pagos_familiar_alquimias_data$FECHA_PAGO = as.Date(mex_pagos_familiar_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(mex_pagos_familiar_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
mex_pagos_familiar_alquimias = mex_pagos_familiar_alquimias_data %>% 
...
# n_alquimias = mex_pagos_familiar_alquimias %>%
#   group_by(ALQUIMIA) %>%
#   tally() # Counts how many elements are in each group
# # Hay 5 alquimias; además, se debe determinar si las observaciones están en fechas consecutivas.
...
alquimias_mes_anterior = mex_pagos_familiar_alquimias %>%
  select(-INSTITUCION) %>%
...
predicciones_finales_familiar_alquimias = bind_rows(alquimias_mes_anterior, predicciones_finales) %>%
  arrange(ALQUIMIA) %>%
...
saveRDS(predicciones_finales_familiar_alquimias, "predicciones_finales_familiar_alquimias.rds")
