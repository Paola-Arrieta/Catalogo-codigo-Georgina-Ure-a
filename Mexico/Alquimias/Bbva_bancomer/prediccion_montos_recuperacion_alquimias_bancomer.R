# ---
# title: "Predicción de montos recuperación: Alquimias de BBVA Bancomer"
...
mex_pagos_bancomer_alquimias_data = dbGetQuery(connection, "
...
# str(mex_pagos_bancomer_alquimias_data)
mex_pagos_bancomer_alquimias_data$FECHA_PAGO = as.Date(mex_pagos_bancomer_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(mex_pagos_bancomer_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
mex_pagos_bancomer_alquimias = mex_pagos_bancomer_alquimias_data %>% 
...
# n_alquimias = mex_pagos_bancomer_alquimias %>%
#   group_by(ALQUIMIA) %>%
...
alquimias_mes_anterior = mex_pagos_bancomer_alquimias %>%
  select(-INSTITUCION) %>%
...
# # Dos alquimias con fechas vacías, pero son muy pocos casos (alquimia 002 con 2 y alquimia 004 con 7).
...
predicciones_finales_bancomer_alquimias = bind_rows(alquimias_mes_anterior, predicciones_finales) %>%
  arrange(ALQUIMIA) %>%
...
saveRDS(predicciones_finales_bancomer_alquimias, "predicciones_finales_bancomer_alquimias.rds")
