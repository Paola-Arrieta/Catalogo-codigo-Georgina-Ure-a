# ---
# title: "Predicción de montos recuperación: Alquimias de Santander"
...
mex_pagos_santander_alquimias_data = dbGetQuery(connection, "
...
# str(mex_pagos_santander_alquimias_data)
mex_pagos_santander_alquimias_data$FECHA_PAGO = as.Date(mex_pagos_santander_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(mex_pagos_santander_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
mex_pagos_santander_alquimias = mex_pagos_santander_alquimias_data %>% 
...
# n_alquimias = mex_pagos_santander_alquimias %>%
#   group_by(ALQUIMIA) %>%
...
alquimias_mes_anterior = mex_pagos_santander_alquimias %>%
  select(-INSTITUCION) %>%
  filter(FECHA_PAGO <= init_last_month)


# Se necesitan algunos meses más para que las alquimias cumplan con las observaciones mínimas para poder realizar un análisis de series de tiempo (24).