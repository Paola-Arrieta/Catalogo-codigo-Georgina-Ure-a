...
slv_pagos_promerica_data = dbGetQuery(connection, "
...
# str(slv_pagos_promerica_data)
slv_pagos_promerica_data$FECHA_PAGO = as.Date(slv_pagos_promerica_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(slv_pagos_promerica_data)
# # Se tienen observaciones desde 2009-08 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

slv_pagos_promerica = slv_pagos_promerica_data %>% 
...
# # por columna
# apply(slv_pagos_promerica, 2, porcentaje_empty)
# # 0.0 de la data en la que no hubo recuperación.

# Se completa la primera columna
slv_pagos_promerica$INSTITUCION = replace_na(as.character(slv_pagos_promerica$INSTITUCION), "A-PROMERICA")
# Se interpolan los valores perdidos
slv_pagos_promerica = slv_pagos_promerica %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# Se decide dejar como primer primer observación el 2010-09-01, para mayor consistencia de la data. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = '2010-09-01'
...
# residuos_data = diff(slv_pagos_promerica$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) no proviene de una distribución normal [(p-value = 0.08784) > (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) no proviene de una distribución normal [(p-values = 0.0004084) < (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) no proviene de una distribución normal [(p-values = 0.00005513) < (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(slv_pagos_promerica$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -2.3856 y un p-value de = 0.4153. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 45 60 90
# # Es así como de determina que los períodos más importantes son cada 45 meses (cada poco menos de cuatro años), cada 60 meses (cada cinco años) y cada 90 meses (cada siete años y medio).
# 
...
# model_calibrated_arima = arima(serie_training, order = c(4, 1, 4), seasonal = list(order = c(1, 0, 1), period = 12))
...
# autoplot(serie_training, xlab = "Fecha", ylab = "", main = "Montos totales de recuperacion para Promérica - El Salvador") +
...
# 
# errors
# # El cuadro detalla los índices de error para cada uno de los modelos planteados. De esta manera se confirma que AIMA calibrado no es la mejor opción, ya que sus valores en MSE y RMSE son muy altos, mientras que Holt-Winters calibrado muestra lo contrario.
# 
...
final_date = slv_pagos_promerica$FECHA_PAGO[nrow(slv_pagos_promerica)]
final_date
...
total_dates = c(slv_pagos_promerica$FECHA_PAGO, dates)

# Predicciones finales con dygraph
predictions = xts(xts(final_predictions, order.by = total_dates))

# dygraph(predictions, width = "100%", ylab = "Monto", main = "Monto de recuperacion para Promérica - El Salvador") %>%
