...
# summary(slv_pagos_davivienda_data)
# # Se tienen observaciones desde 2019-11 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).
...
# # por columna
# apply(slv_pagos_davivienda, 2, porcentaje_empty)
# # 10.20 de la data en la que no hubo recuperación.
...
# residuos_data = diff(slv_pagos_davivienda$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) no proviene de una distribución normal [(p-value = 0.0000000001818) < (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) no proviene de una distribución normal [(p-values = 0.0000000802) < (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) no proviene de una distribución normal [(p-values = 0.000000005269) < (alfa = 0.05)].
...
# # Se obtiene un valor de -2.1377 y un p-value de = 0.5191. La hipótesis nula no es aceptada y, por lo tanto, la serie es estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 24 16 12
# # Es así como de determina que los períodos más importantes son cada 24 meses (cada dos años), cada 16 meses (cada poco más de un año) y cada 12 meses (cada año).
# 
...
# model_arima = arima(serie_training, order = c(2,2,1), seasonal = list(order = c(0,0,0), period = 12))
# model_arima
# 
...
# model_calibrated_arima = arima(serie_training, order = c(0, 0, 0), seasonal = list(order = c(1, 0, 0), period = 12))
...
# autoplot(serie_training, xlab = "Fecha", ylab = "", main = "Montos totales de recuperacion para Davivienda - El Salvador") +
...
# dygraph(predictions, width = "100%", ylab = "Monto", main = "Monto de recuperacion para Davivienda - El Salvador") %>%
