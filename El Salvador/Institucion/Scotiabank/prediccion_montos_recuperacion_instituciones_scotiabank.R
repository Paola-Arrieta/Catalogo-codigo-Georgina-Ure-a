...
slv_pagos_scotiabank_data = dbGetQuery(connection, "
...
# str(slv_pagos_scotiabank_data)
slv_pagos_scotiabank_data$FECHA_PAGO = as.Date(slv_pagos_scotiabank_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(slv_pagos_scotiabank_data)
# # Se tienen observaciones desde 2018-06 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

slv_pagos_scotiabank = slv_pagos_scotiabank_data %>% 
...
# # por columna
# apply(slv_pagos_scotiabank, 2, porcentaje_empty)
# # 0.0 de la data en la que no hubo recuperación.

# Se completa la primera columna
slv_pagos_scotiabank$INSTITUCION = replace_na(as.character(slv_pagos_scotiabank$INSTITUCION), "SCOTIABANK")
# Se interpolan los valores perdidos
slv_pagos_scotiabank = slv_pagos_scotiabank %>%
...
init_first_month = slv_pagos_scotiabank$FECHA_PAGO[1]
...
# residuos_data = diff(slv_pagos_scotiabank$MONTO_TOTAL)
# 
...
# # El histograma anterior presenta la distribución de los residuos para las observaciones. Se observa una distribución medio asimétrica negativa, lo que podría indicar que tal vez incluye valores atípicos.
# 
...
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) no proviene de una distribución normal [(p-value =  0.2258) > (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) no proviene de una distribución normal [(p-values = 0.002911) < (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) no proviene de una distribución normal [(p-values = 0.004439) < (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(slv_pagos_scotiabank$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -2.0394 y un p-value de = 0.559. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 36 24 18
# # Es así como de determina que los períodos más importantes son cada 36 meses (cada tres años), cada 24 meses (cada dos años) y cada 18 meses (cada año y medio).
# 
...
# model_calibrated_arima = arima(serie_training, order = c(3, 4, 0), seasonal = list(order = c(1, 0, 0), period = 12))
...
# autoplot(serie_training, xlab = "Fecha", ylab = "", main = "Montos totales de recuperacion para Scotiabank - El Salvador") +
...
final_date = slv_pagos_scotiabank$FECHA_PAGO[nrow(slv_pagos_scotiabank)]
final_date
...
total_dates = c(slv_pagos_scotiabank$FECHA_PAGO, dates)

# Predicciones finales con dygraph
predictions = xts(xts(final_predictions, order.by = total_dates))

# dygraph(predictions, width = "100%", ylab = "Monto", main = "Monto de recuperacion para Scotiabank - El Salvador") %>%
...
#   dyAxis("y", valueRange = c(0, 100000))
