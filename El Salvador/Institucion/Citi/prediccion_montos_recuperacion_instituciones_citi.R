...
slv_pagos_citi_data = dbGetQuery(connection, "
...
# str(slv_pagos_citi_data)
slv_pagos_citi_data$FECHA_PAGO = as.Date(slv_pagos_citi_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(slv_pagos_citi_data)
...
slv_pagos_citi = slv_pagos_citi_data %>% 
...
# # por columna
# apply(slv_pagos_citi, 2, porcentaje_empty)
# # 0.0 de la data en la que no hubo recuperación.

# Se completa la primera columna
slv_pagos_citi$INSTITUCION = replace_na(as.character(slv_pagos_citi$INSTITUCION), "CITI")
# Se interpolan los valores perdidos
slv_pagos_citi = slv_pagos_citi %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# Se decide dejar como primer registro 2018-01, ya que es cuando las caídas presentan picos menos pronunciados. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = '2018-01-01'
...
# residuos_data = diff(slv_pagos_citi$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) no proviene de una distribución normal [(p-value = 0.5319) > (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) no proviene de una distribución normal [(p-values = 0.5498) > (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) no proviene de una distribución normal [(p-values = 0.3871) > (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(slv_pagos_citi$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -2.8569 y un p-value de = 0.2268. La hipótesis nula es aceptada y, por lo tanto, la serie no es estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 14.4 36.0 18.0
# # Es así como de determina que los períodos más importantes son cada 14.4 meses (cada poco más de un año), cada 36.00 meses (cada tres años) y cada 18.0 meses (cada año y medio).
# 
...
# model_arima = arima(serie_training, order = c(1,1,0), seasonal = list(order = c(1,0,0), period = 12))
# model_arima
# 
...
# model_calibrated_arima = arima(serie_training, order = c(1, 4, 2), seasonal = list(order = c(0, 0, 0), period = 12))
...
# autoplot(serie_training, xlab = "Fecha", ylab = "", main = "Montos totales de recuperacion para Citi - El Salvador") +
...
# 
# # Se confirma, nuevamente, que el mejor modelo es Holt-Winters calibrado al igual que ARIMA, porque muestran valores más bajos para MSE y RMSE.
...
final_date = slv_pagos_citi$FECHA_PAGO[nrow(slv_pagos_citi)]
final_date
...
total_dates = c(slv_pagos_citi$FECHA_PAGO, dates)

# Predicciones finales con dygraph
predictions = xts(xts(final_predictions, order.by = total_dates))

# dygraph(predictions, width = "100%", ylab = "Monto", main = "Monto de recuperacion para Citi - El Salvador") %>%
...
#   dyAxis("y", valueRange = c(0, 200000))
