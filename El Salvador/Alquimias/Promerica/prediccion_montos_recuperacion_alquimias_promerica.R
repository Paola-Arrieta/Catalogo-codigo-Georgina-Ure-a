...
slv_pagos_promerica_alquimias_data = dbGetQuery(connection, "
...
# str(slv_pagos_promerica_alquimias_data)
slv_pagos_promerica_alquimias_data$FECHA_PAGO = as.Date(slv_pagos_promerica_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(slv_pagos_promerica_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
slv_pagos_promerica_alquimias = slv_pagos_promerica_alquimias_data %>% 
...
# n_alquimias = slv_pagos_promerica_alquimias %>%
#   group_by(ALQUIMIA) %>%
#   tally() # Counts how many elements are in each group
# # Hay 106 alquimias; además, se debe determinar si las observaciones están en fechas consecutivas.
...
alquimias_mes_anterior = slv_pagos_promerica_alquimias %>%
  select(-INSTITUCION) %>%
...
# # Hay 9 alquimias con 10 o más 5 fechas vacías.
