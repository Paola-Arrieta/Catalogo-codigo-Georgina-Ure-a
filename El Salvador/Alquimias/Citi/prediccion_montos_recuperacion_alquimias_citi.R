...
slv_pagos_citi_alquimias_data = dbGetQuery(connection, "
...
# str(slv_pagos_citi_alquimias_data)
slv_pagos_citi_alquimias_data$FECHA_PAGO = as.Date(slv_pagos_citi_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(slv_pagos_citi_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
slv_pagos_citi_alquimias = slv_pagos_citi_alquimias_data %>% 
...
# n_alquimias = slv_pagos_citi_alquimias %>%
#   group_by(ALQUIMIA) %>%
...
alquimias_mes_anterior = slv_pagos_citi_alquimias %>%
  select(-INSTITUCION) %>%
