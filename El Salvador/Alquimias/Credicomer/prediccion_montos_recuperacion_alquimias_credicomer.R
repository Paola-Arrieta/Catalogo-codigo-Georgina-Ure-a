# ---
# title: "Predicción de montos recuperación: Alquimias de Credicomer"
...
slv_pagos_credicomer_alquimias_data = dbGetQuery(connection, "
...
# str(slv_pagos_credicomer_alquimias_data)
slv_pagos_credicomer_alquimias_data$FECHA_PAGO = as.Date(slv_pagos_credicomer_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(slv_pagos_credicomer_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
slv_pagos_credicomer_alquimias = slv_pagos_credicomer_alquimias_data %>% 
...
# n_alquimias = slv_pagos_credicomer_alquimias %>%
#   group_by(ALQUIMIA) %>%
...
alquimias_mes_anterior = slv_pagos_credicomer_alquimias %>%
  select(-INSTITUCION) %>%
...
# Se necesitan algunos meses más para que las alquimias de la institución cumplan con las observaciones mínimas para poder realizar un análisis de series de tiempo (24).