# ---
# title: "Predicción de montos recuperación Scotiabank"
...
# str(dom_pagos_scotiabank_data)
dom_pagos_scotiabank_data$FECHA_PAGO = as.Date(dom_pagos_scotiabank_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(dom_pagos_scotiabank_data)
# # Se tienen observaciones desde 2012-05 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

dom_pagos_scotiabank = dom_pagos_scotiabank_data %>% 
...
# # por columna
# apply(dom_pagos_scotiabank, 2, porcentaje_empty)
# # 0.0% de la data en la que no hubo recuperación.

# Se completa la primera columna
dom_pagos_scotiabank$INSTITUCION = replace_na(as.character(dom_pagos_scotiabank$INSTITUCION), "SCOTIABANK")
# Se interpolan los valores perdidos
dom_pagos_scotiabank = dom_pagos_scotiabank %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# Se decide dejar como primer registro la primera observación. Además, decido dejar como última observación el mes anterior al actual completo.  
init_first_month = dom_pagos_scotiabank$FECHA_PAGO[1]
...
# residuos_data = diff(dom_pagos_scotiabank$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.006687) < (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# nortest::lillie.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.008539) < (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# nortest::cvm.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.00593) < (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(dom_pagos_scotiabank$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -3.625 y un p-value de 0.03363. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 48.000000  2.823529  2.440678
# # Es así como de determina que los períodos más importantes son cada 48.0 meses (cada cuatro años), cada 2.82 meses (cada casi trimestre) y cada 2.44 meses (cada bimestre).
# 
...
# model_calibrated_arima = arima(serie_training, order = c(4, 3, 0), seasonal = list(order = c(1, 1, 1), period = 12))
...
# autoplot(serie_training, xlab = "Fecha", ylab = "", main = "Montos totales de recuperacion para Scotiabank - República Dominicana") +
...
# 
# # Se confirma, nuevamente, que el mejor modelo es Holt-Winters calibrado porque muestra valores más bajos para MSE y RMSE y PTFA. No obstante, cabe destacar que ARIMA se asemeja mucho a este último.
...
final_date = dom_pagos_scotiabank$FECHA_PAGO[nrow(dom_pagos_scotiabank)]
final_date
...
total_dates = c(dom_pagos_scotiabank$FECHA_PAGO, dates)

# Predicciones finales con dygraph
predictions = xts(xts(final_predictions, order.by = total_dates))

# dygraphs::dygraph(predictions, width = "100%", ylab = "Monto", main = "Monto de recuperacion para Scotiabank - República Dominicana") %>%
...
#   dygraphs::dyAxis("y", valueRange = c(0, 1000000))
...
predicciones_finales_scotiabank = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_scotiabank) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_scotiabank = predicciones_finales_scotiabank %>%
...
saveRDS(predicciones_finales_scotiabank, "predicciones_finales_scotiabank.rds")
