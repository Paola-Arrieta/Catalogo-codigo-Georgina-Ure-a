# ---
# title: "Predicción de montos recuperación Promérica"
...
dom_pagos_promerica_data = dbGetQuery(connection, "
...
# str(dom_pagos_promerica_data)
dom_pagos_promerica_data$FECHA_PAGO = as.Date(dom_pagos_promerica_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(dom_pagos_promerica_data)
# # Se tienen observaciones desde 2011-10 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

dom_pagos_promerica = dom_pagos_promerica_data %>% 
...
# # por columna
# apply(dom_pagos_promerica, 2, porcentaje_empty)
# # 0.0% de la data en la que no hubo recuperación.

# Se completa la primera columna
dom_pagos_promerica$INSTITUCION = replace_na(as.character(dom_pagos_promerica$INSTITUCION), "A-PROMERICA")
# Se interpolan los valores perdidos
dom_pagos_promerica = dom_pagos_promerica %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# Se decide dejar como primer registro 2012-01-01, ya que es cuando los montos presentan mayor estabilidad. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = '2012-01-01'
...
# residuos_data = diff(dom_pagos_promerica$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.4616) > (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# nortest::lillie.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.1561) > (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# nortest::cvm.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.315) > (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(dom_pagos_promerica$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -3.5938 y un p-value de 0.03639. La hipótesis nula no es aceptada y, por lo tanto, la serie es estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 48.0 72.0  2.4# # Es así como de determina que los períodos más importantes son cada 48.0 meses (cada cuatro años), cada 72.0 meses (cada seis años) y cada 2.4 meses (cada bimestre).
# 
...
# model_arima = arima(serie_training, order = c(1,1,1), seasonal = list(order = c(0,0,2), period = 12))
# model_arima
# 
...
# model_calibrated_arima = arima(serie_training, order = c(0, 3, 1), seasonal = list(order = c(0, 0, 1), period = 12))
...
# autoplot(serie_training, xlab = "Fecha", ylab = "", main = "Montos totales de recuperacion para Promérica - República Dominicana") +
...
final_date = dom_pagos_promerica$FECHA_PAGO[nrow(dom_pagos_promerica)]
final_date
...
total_dates = c(dom_pagos_promerica$FECHA_PAGO, dates)

# Predicciones finales con dygraph
predictions = xts(xts(final_predictions, order.by = total_dates))

# dygraphs::dygraph(predictions, width = "100%", ylab = "Monto", main = "Monto de recuperacion para Promérica - República Dominicana") %>%
...
#   dygraphs::dyAxis("y", valueRange = c(0, 15000000))
...
predicciones_finales_promerica = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_promerica) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_promerica = predicciones_finales_promerica %>%
...
saveRDS(predicciones_finales_promerica, "predicciones_finales_promerica.rds")
