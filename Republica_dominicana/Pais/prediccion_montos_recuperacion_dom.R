...
# --- ESCENARIO 1: SERIE COMPLETA --- #
# --- NIVEL PAÍS REPÚBLICA DOMINICANA --- #
...
dominicana_pagos = dbGetQuery(connection, "
...
# str(dominicana_pagos)
dominicana_pagos$FECHA_PAGO = as.Date(dominicana_pagos$FECHA_PAGO,format="%d/%m/%Y")
# summary(dominicana_pagos)
# # Se tienen observaciones desde 2011-10 a 2023 (no equiespaciadas o consecutivas)

# dominicana_pagos = dominicana_pagos %>%
#   arrange(MONTO)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso). 
dom_pagos = dominicana_pagos %>% 
...
# por columna
# apply(dom_pagos, 2, porcentaje_empty)
# 0.0% de la data en la que no hubo recuperación.

# Se decide, entonces, empezar la serie desde su primera observación y dejar como última observación el mes anterior al actual completo.  
...
# residuos_data = diff(dom_pagos$MONTO_TOTAL)
# 
...
# # El histograma anterior presenta la distribución de los residuos para las observaciones. Se observa una distribución medio asimétrica positiva, lo que podría indicar que podría incluir valores atípicos.
# 
...
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.09411) > (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.108) > (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, se rechaza la hipótesis nula: Existe suficiente evidencia estadística para rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.02839) < (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(dom_pagos$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
# El objeto creado anteriormente inicia en 2935605 como monto de pago por todos los clientes de República Dominicana y finaliza en  23341219 en octubre 2023. Es decir, es una sucesión ordenada de valores.
...
# # Se obtiene un valor de -0.33053 y un p-value de 0.9881. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.  
...
# periods
# # 20.57143 24.00000 72.00000
# # Es así como de determina que los períodos más importantes son cada 20.57 meses (cada poco más de año y medio), cada 24.00 meses (cada dos años) y cada 72.00 meses (cada seis años).
# 
...
# autoplot(serie_training, xlab = "Fecha", ylab = "", main = "Montos totales de recuperacion para República Dominicana") +
...
# 
# errors
# # El cuadro anterior detalla los índices de error para cada uno de los modelos planteados. De esta manera se confirma que ni ARIMA ni Arima calibrado son la mejor opción, ya que sus valores en MSE y RMSE son muy altos, mientras que Holt-Winters calibrado muestra lo contrario.
# 
...
# 
# # Se confirma, nuevamente, que ARIMA calibrado no es el mejor modelo, mientras que Holt-Winters calibrado muestra índices más bajos tanto en MSE y RMSE.
...
# De esta manera se muestran las predicciones para los últimos meses de este 2023 y los primeros del 2024, así como sus respectivos intervalos con 95% de confianza.
...
final_date = dom_pagos$FECHA_PAGO[nrow(dom_pagos)]
final_date
...
total_dates = c(dom_pagos$FECHA_PAGO, dates)

# Predicciones finales con dygraph
predictions = xts(xts(final_predictions, order.by = total_dates))

# dygraph(predictions, width = "100%", ylab = "Monto", main = "Monto de recuperacion para República Dominicana") %>%
...
#   dyAxis("y", valueRange = c(0,40000000))
# 
...
predicciones_finales_dom = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame() 
names(predicciones_finales_dom) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")
predicciones_finales_dom = predicciones_finales_dom %>%
...
saveRDS(predicciones_finales_dom, "predicciones_finales_dom.rds")
