...
dom_pagos_scotiabank_alquimias_data = dbGetQuery(connection, "
...
# str(dom_pagos_scotiabank_alquimias_data)
dom_pagos_scotiabank_alquimias_data$FECHA_PAGO = as.Date(dom_pagos_scotiabank_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(dom_pagos_scotiabank_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
dom_pagos_scotiabank_alquimias = dom_pagos_scotiabank_alquimias_data %>% 
...
# n_alquimias = dom_pagos_scotiabank_alquimias %>%
#   group_by(ALQUIMIA) %>%
...
alquimias_mes_anterior = dom_pagos_scotiabank_alquimias %>%
  select(-INSTITUCION) %>%
...
# # La alquimia no posee fechas vacías.
