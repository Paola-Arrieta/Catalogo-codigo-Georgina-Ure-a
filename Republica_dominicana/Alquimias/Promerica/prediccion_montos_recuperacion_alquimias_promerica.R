...
dom_pagos_promerica_alquimias_data = dbGetQuery(connection, "
...
# str(dom_pagos_promerica_alquimias_data)
dom_pagos_promerica_alquimias_data$FECHA_PAGO = as.Date(dom_pagos_promerica_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(dom_pagos_promerica_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
dom_pagos_promerica_alquimias = dom_pagos_promerica_alquimias_data %>% 
...
# n_alquimias = dom_pagos_promerica_alquimias %>%
#   group_by(ALQUIMIA) %>%
#   tally() # Counts how many elements are in each group
# # Hay 49 alquimias; además, se debe determinar si las observaciones están en fechas consecutivas.
...
alquimias_mes_anterior = dom_pagos_promerica_alquimias %>%
  select(-INSTITUCION) %>%
...
# # Hay dos alquimis con más de 10 fechas vacías.
