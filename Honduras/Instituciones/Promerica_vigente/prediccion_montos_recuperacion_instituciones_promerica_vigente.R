# ---
# title: "Predicción de montos recuperación Promérica vigente"
...
hnd_pagos_promerica_vigente_data = dbGetQuery(connection, "
                                 ...
                                 WHERE PAI_NOMBRE = 'HONDURAS'
                                  ...
                                  AND INSTITUCION = 'A-PROMERICA VIGENTE' 
                                  ")
...
# str(hnd_pagos_promerica_vigente_data)
hnd_pagos_promerica_vigente_data$FECHA_PAGO = as.Date(hnd_pagos_promerica_vigente_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(hnd_pagos_promerica_vigente_data)
# # Se tienen observaciones desde 2017-10 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

hnd_pagos_promerica_vigente = hnd_pagos_promerica_vigente_data %>% 
...
# # por columna
# apply(hnd_pagos_promerica_vigente, 2, porcentaje_empty)
# # 8.10% de la data en la que no hubo recuperación.

# Se completa la primera columna
hnd_pagos_promerica_vigente$INSTITUCION = replace_na(as.character(hnd_pagos_promerica_vigente$INSTITUCION), "A-PROMERICA VIGENTE")
# Se interpolan los valores perdidos
hnd_pagos_promerica_vigente = hnd_pagos_promerica_vigente %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# Se decide dejar como primer registro 2018-08-01, ya que es cuando los montos presentan mayor estabilidad y similitud. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = '2018-08-01'
...
# residuos_data = diff(hnd_pagos_promerica_vigente$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.4788) > (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# nortest::lillie.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.4343) > (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# nortest::cvm.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.1658) > (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(hnd_pagos_promerica_vigente$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -1.8516 y un p-value de 0.635. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 32.00000 21.33333 16.00000
# # Es así como de determina que los períodos más importantes son cada 32.00 meses (cada poco más de dos años y medio), cada 21.33 meses (cada poco menos de dos años) y cada 16.00 meses (cada poco menos de un año y medio).
# 
...
# model_calibrated_arima = arima(serie_training, order = c(0, 0, 0), seasonal = list(order = c(0, 1, 0), period = 12))
...
# autoplot(serie_training, xlab = "Fecha", ylab = "", main = "Montos totales de recuperacion para Honduras - Promérica vigente") +
...
# 
# errors
# # El cuadro detalla los índices de error para cada uno de los modelos planteados. De esta manera se confirma que Holt-Winters no es la mejor opción, ya que sus valores en MSE y RMSE son muy altos, mientras que Holt-Winters calibrado muestra lo contrario.
# 
...
final_date = hnd_pagos_promerica_vigente$FECHA_PAGO[nrow(hnd_pagos_promerica_vigente)]
final_date
...
total_dates = c(hnd_pagos_promerica_vigente$FECHA_PAGO, dates)

# Predicciones finales con dygraph
predictions = xts(xts(final_predictions, order.by = total_dates))

# dygraphs::dygraph(predictions, width = "100%", ylab = "Monto", main = "Monto de recuperacion para Honduras - Promérica vigente") %>%
...
#   dygraphs::dyAxis("y", valueRange = c(0, 3000000))
...
predicciones_finales_promerica_vigente = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_promerica_vigente) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_promerica_vigente = predicciones_finales_promerica_vigente %>%
...
saveRDS(predicciones_finales_promerica_vigente, "predicciones_finales_promerica_vigente.rds")
