# ---
# title: "Predicción de montos recuperación Promérica castigada"
...
# str(hnd_pagos_promerica_castigada_data)
hnd_pagos_promerica_castigada_data$FECHA_PAGO = as.Date(hnd_pagos_promerica_castigada_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(hnd_pagos_promerica_castigada_data)
# # Se tienen observaciones desde 2018-01 hasta 2023 (no necesariamente equiespaciadas o consecutivas, por lo que hay que cerciorarse de que esta condición se cumpla; de no ser así se puede plantear recurrir a la imputación).

hnd_pagos_promerica_castigada = hnd_pagos_promerica_castigada_data %>% 
...
# # por columna
# apply(hnd_pagos_promerica_castigada, 2, porcentaje_empty)
# # 4.22% de la data en la que no hubo recuperación.

# Se completa la primera columna
hnd_pagos_promerica_castigada$INSTITUCION = replace_na(as.character(hnd_pagos_promerica_castigada$INSTITUCION), "A-PROMERICA-CASTIGADA")
# Se interpolan los valores perdidos
hnd_pagos_promerica_castigada = hnd_pagos_promerica_castigada %>%
  mutate(MONTO_TOTAL=na.approx(MONTO_TOTAL))

# Se decide dejar como primer registro 2018-06-01, ya que es cuando los montos presentan mayor estabilidad y similitud. Decido dejar como última observación el mes anterior al actual completo.  
init_first_month = '2018-06-01'
...
# residuos_data = diff(hnd_pagos_promerica_castigada$MONTO_TOTAL)
# 
...
# # El histograma anterior presenta la distribución de los residuos para las observaciones. Se observa una distribución medio asimétrica positiva, lo que podría indicar que tal vez incluye valores atípicos.
# 
...
# nortest::pearson.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.4838) > (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# nortest::lillie.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.436) > (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# nortest::cvm.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.284) > (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(hnd_pagos_promerica_castigada$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
...
# # Se obtiene un valor de -2.0846 y un p-value de 0.5407. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.
...
# periods
# # 36.00000 14.40000 10.28571
# # Es así como de determina que los períodos más importantes son cada 36.10 meses (cada tres años), cada 14.40 meses (cada poco más de un año) y cada 10.28 meses (cada poco menos de un año).
# 
# # Periodograma con el paquete TSA
# res = TSA::periodogram(serie)
...
# model_arima = arima(serie_training, order = c(1,0,0), seasonal = list(order = c(0,0,0), period = 12))
# model_arima
# 
...
# model_calibrated_arima = arima(serie_training, order = c(0, 2, 0), seasonal = list(order = c(0, 0, 1), period = 12))
...
# autoplot(serie_training, xlab = "Fecha", ylab = "", main = "Montos totales de recuperacion para Honduras - Promérica castigada") +
...
#   ggplot2::guides(colour = ggplot2::guide_legend(title = "Predicciones"))
# 
# # Errores
# 
...
# 
#   res <- reshape::melt(t(centros), varnames = c("E", "Modelos"))
...
# 
# # Se confirma, nuevamente, que el mejor modelo es Holt-Winters calibrado porque muestra valores más bajos para MSE y RMSE.
...
final_date = hnd_pagos_promerica_castigada$FECHA_PAGO[nrow(hnd_pagos_promerica_castigada)]
final_date
...
total_dates = c(hnd_pagos_promerica_castigada$FECHA_PAGO, dates)

# Predicciones finales con dygraph
predictions = xts(xts(final_predictions, order.by = total_dates))

# dygraphs::dygraph(predictions, width = "100%", ylab = "Monto", main = "Monto de recuperacion para Honduras - Promérica castigada") %>%
#   dygraphs::dySeries(c("p.LowerConfidenceLimit", "p.prediction", "p.UpperConfidenceLimit"), label = "Prediction") %>%
#   dygraphs::dyRangeSelector(height = 20, strokeColor = "Gray") %>%
#   dygraphs::dyOptions(maxNumberWidth = 20) %>%
#   dygraphs::dyAxis("y", valueRange = c(0, 5000000))
...
predicciones_finales_promerica_castigada = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame()
names(predicciones_finales_promerica_castigada) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")

predicciones_finales_promerica_castigada = predicciones_finales_promerica_castigada %>%
...
saveRDS(predicciones_finales_promerica_castigada, "predicciones_finales_promerica_castigada.rds")
