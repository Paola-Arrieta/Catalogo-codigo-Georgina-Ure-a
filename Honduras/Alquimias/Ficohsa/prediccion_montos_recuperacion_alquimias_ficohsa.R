...
hnd_pagos_ficohsa_alquimias_data = dbGetQuery(connection, "
...
# str(hnd_pagos_ficohsa_alquimias_data)
hnd_pagos_ficohsa_alquimias_data$FECHA_PAGO = as.Date(hnd_pagos_ficohsa_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(hnd_pagos_ficohsa_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
hnd_pagos_ficohsa_alquimias = hnd_pagos_ficohsa_alquimias_data %>% 
...
# n_alquimias = hnd_pagos_ficohsa_alquimias %>%
#   group_by(ALQUIMIA) %>%
...
alquimias_mes_anterior = hnd_pagos_ficohsa_alquimias %>%
  select(-INSTITUCION) %>%
...
# Se necesitan algunos meses más para que la alquimia de la institución cumplan con el mínimo de observaciones para poder realizar un análisis de series de tiempo (24).