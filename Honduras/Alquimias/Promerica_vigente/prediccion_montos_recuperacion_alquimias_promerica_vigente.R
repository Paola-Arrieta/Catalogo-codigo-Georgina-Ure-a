# ---
# title: "Predicción de montos recuperación: Alquimias de Promérica vigente"
...
hnd_pagos_promerica_vigente_alquimias_data = dbGetQuery(connection, "
...
# str(hnd_pagos_promerica_vigente_alquimias_data)
hnd_pagos_promerica_vigente_alquimias_data$FECHA_PAGO = as.Date(hnd_pagos_promerica_vigente_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(hnd_pagos_promerica_vigente_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
hnd_pagos_promerica_vigente_alquimias = hnd_pagos_promerica_vigente_alquimias_data %>% 
...
# n_alquimias = hnd_pagos_promerica_vigente_alquimias %>%
#   group_by(ALQUIMIA) %>%
#   tally() # Counts how many elements are in each group
# # Hay 4 alquimias; además, se debe determinar si las observaciones están en fechas consecutivas.
...
alquimias_mes_anterior = hnd_pagos_promerica_vigente_alquimias %>%
  select(-INSTITUCION) %>%
...
# # Hay 2 alquimias con fechas vacías, una de ellas con 8 y la otra con solamente 1.
...
predicciones_finales_promerica_vigente_alquimias = bind_rows(alquimias_mes_anterior, predicciones_finales) %>%
  arrange(ALQUIMIA) %>%
...
saveRDS(predicciones_finales_promerica_vigente_alquimias, "predicciones_finales_promerica_vigente_alquimias.rds")
