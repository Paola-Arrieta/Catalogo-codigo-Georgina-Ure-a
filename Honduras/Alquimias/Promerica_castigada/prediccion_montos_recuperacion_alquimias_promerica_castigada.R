# ---
# title: "Predicción de montos recuperación: Alquimias de Promérica castigada"
...
hnd_pagos_promerica_castigada_alquimias_data = dbGetQuery(connection, "
...
# str(hnd_pagos_promerica_castigada_alquimias_data)
hnd_pagos_promerica_castigada_alquimias_data$FECHA_PAGO = as.Date(hnd_pagos_promerica_castigada_alquimias_data$FECHA_PAGO,format="%d/%m/%Y")
# summary(hnd_pagos_promerica_castigada_alquimias_data)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso) y no por cliente.
hnd_pagos_promerica_castigada_alquimias = hnd_pagos_promerica_castigada_alquimias_data %>% 
...
# n_alquimias = hnd_pagos_promerica_castigada_alquimias %>%
#   group_by(ALQUIMIA) %>%
#   tally() # Counts how many elements are in each group
# # Hay 9 alquimias; además, se debe determinar si las observaciones están en fechas consecutivas.
...
alquimias_mes_anterior = hnd_pagos_promerica_castigada_alquimias %>%
  select(-INSTITUCION) %>%
...
# # Hay 2 alquimias con fechas vacías, una de ellas con 3 y la otra con solamente 1.
...
predicciones_finales_promerica_castigada_alquimias = bind_rows(alquimias_mes_anterior, predicciones_finales) %>%
  arrange(ALQUIMIA) %>%
...
saveRDS(predicciones_finales_promerica_castigada_alquimias, "predicciones_finales_promerica_castigada_alquimias.rds")
