...
honduras_pagos = dbGetQuery(connection, "
...
# str(honduras_pagos)
honduras_pagos$FECHA_PAGO = as.Date(honduras_pagos$FECHA_PAGO,format="%d/%m/%Y")
# summary(honduras_pagos)
# # Se tienen observaciones desde 2017-10 a 2023 (no equiespaciadas o consecutivas necesariamente)

# honduras_pagos = honduras_pagos %>%
#   arrange(MONTO)

# Se agrupa el total de pagos por fecha, ya que el objetivo es pronosticar de forma global (en este caso). 
hnd_pagos = honduras_pagos %>% 
...
# por columna
# apply(hnd_pagos, 2, porcentaje_empty)
# 6.75% de la data en la que no hubo recuperación.

# Se decide, empezar la serie en 2018-06, ya que en este punto se presenta mayor constancia a lo largo del tiempo hasta la actualidad, y dejar como última observación el mes anterior al actual completo.  
...
# residuos_data = diff(hnd_pagos$MONTO_TOTAL)
# 
...
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-value = 0.05794) > (alfa = 0.05)].
# 
# # Prueba de normalidad Lilliefors (Kolmogorov - Smirnov)
# lillie.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.3464) > (alfa = 0.05)].
# 
# # Prueba de normalidad Cramer-von Mises
# cvm.test(residuos_data)
# # Según la prueba anterior, no se rechaza la hipótesis nula: Existe suficiente evidencia estadística para no rechazar la hipótesis nula de que la muestra (los residuos de la serie) proviene de una distribución normal [(p-values = 0.2431) > (alfa = 0.05)].


# Serie de tiempo

# Se crea la serie de tiempo. El patrón estacional es mensual, por eso la frecuencia es 12
serie = ts(hnd_pagos$MONTO_TOTAL, start = c(init_first_month_year,init_first_month_month), frequency = 12)
# str(serie)
# head(serie)
# tail(serie)
# El objeto creado anteriormente inicia en 1442390 como monto de pago por todos los clientes de Honduras y finaliza en  5133811 en octubre 2023. Es decir, es una sucesión ordenada de valores.
...
# # Se obtiene un valor de -1.8705 y un p-value de 0.6275. La hipótesis nula es aceptada y, por lo tanto, la serie es no estacionaria. In other words, it has some time-dependent structure and does not have constant variance over time.  
...
# periods
# # 36.0 24.0 14.4
# # Es así como de determina que los períodos más importantes son cada 36.0 meses (cada 3 años), cada 24.0 meses (cada dos años) y cada 14.4 meses (cada poco más de un año).
# 
...
# model_calibrated_arima = arima(serie_training, order = c(0, 5, 2), seasonal = list(order = c(1, 0, 1), period = 12))
...
# autoplot(serie_training, xlab = "Fecha", ylab = "", main = "Montos totales de recuperacion para Honduras") +
...
# 
# # Se confirma, nuevamente, que el mejor modelo es Holt-Winters calibrado porque muestra valores más bajos para MSE y RMSE. No obstante, cabe mencionar que Holt-Winters es sumamente parecido, al igual que ARIMA.
...
final_date = hnd_pagos$FECHA_PAGO[nrow(hnd_pagos)]
final_date
...
total_dates = c(hnd_pagos$FECHA_PAGO, dates)

# Predicciones finales con dygraph
predictions = xts(xts(final_predictions, order.by = total_dates))

# dygraph(predictions, width = "100%", ylab = "Monto", main = "Monto de recuperacion para Honduras") %>%
...
#   dyAxis("y", valueRange = c(0,12000000))
# 
...
predicciones_finales_hnd = cbind(fechas_finales,predicciones_finales) %>%
  as.data.frame() 
names(predicciones_finales_hnd) = c("Fecha", "Monto_real", "Estimacion_puntual", "Limite_confianza_inferior", "Limite_confianza_superior")
predicciones_finales_hnd = predicciones_finales_hnd %>%
...
saveRDS(predicciones_finales_hnd, "predicciones_finales_hnd.rds")
